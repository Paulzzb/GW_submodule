#!/bin/bash -l
#SBATCH -J bench_gw_large
#SBATCH -t 96:00:00
#SBATCH -p normal
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -c 40
#SBATCH -o log/gw_large-%j.out
#SBATCH -e log/gw_large-%j.err
#
# GW for large cells: Si64, STO3_8 (MATLAB): input_driver + qp.launcher.
#
# Prerequisites: QE ground state finished (see slurm_qe_gs_large.sh).
#
...
#
# Overrides:
#   MATLAB_BIN=/path/to/matlab sbatch slurm_gw_large.sh
#   CASES="Si64" NAMELIST=test_dir sbatch slurm_gw_large.sh
#   REPO_ROOT=/path/to/hefeikssolvtopub sbatch slurm_gw_large.sh
#
...
echo "[$(date)] GW benchmarks (large cells)"
...
for case in $CASES; do
  ...
  if [[ -n "$gs_rel" && ! -d "$gs_rel" ]]; then
    ...
    echo "      Run slurm_qe_gs_large.sh first (expect Si64.save / STO3.save)." >&2
  fi
...
cd "$BENCH_ROOT"
echo
echo "[$(date)] Large-cell GW finished."
