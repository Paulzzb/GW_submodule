...
#
# Prerequisites: QE ground state finished (see qe_large.sh).
#
...
#
# Overrides:
#   MATLAB_BIN=/path/to/matlab sbatch gw_large.sh
#   CASES="Si64" NAMELIST=test_dir sbatch gw_large.sh
#   REPO_ROOT=/path/to/repo sbatch gw_large.sh
#
...
for case in $CASES; do
  ...
  if [[ -z "$gs_rel" || ! -d "$gs_rel" ]]; then
    ...
    echo "      Run qe_large.sh first (expect Si64.save / STO3.save)." >&2
    exit 1
  fi
