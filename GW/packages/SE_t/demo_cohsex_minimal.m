...
E_f = zeros(max(Se.NB(2), 8), n_ibz, 1, 'single');
for ib = Se.NB(1):Se.NB(2)
    E_f(ib, 1, 1) = single(0.65);   % 0 < f < 1  =>  non-zero (2*pi*pre*(1 - 2*f/spin_occ*...))
end
...
for i_count = 1:double(Se.basis.NS_todo)
    ...
    rng(1 + i_bas, 'twister');
    gamp = complex(randn(ng, ng, 'single'), randn(ng, ng, 'single')) * single(0.02);
    ...
    for iq = 1:n_qbz
        ...
        M_chi = complex(randn(ng, ng, 'single'), randn(ng, ng, 'single')) * single(0.01);
        ...
        for ib = double(Se.NB(1)):double(Se.NB(2))
            ...
            % -------------------------------------------------------------
            rho = complex(randn(ng, 1, 'single'), randn(ng, 1, 'single')) * single(0.1);
    ...
    net = DP_Sc(1) + DP_Sc(2);
    Se.SF(i_bas, 1) = Se.SF(i_bas, 1) + complex(single(net), single(0));
