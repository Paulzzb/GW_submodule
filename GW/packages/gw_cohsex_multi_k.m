...
%   - Single IBZ k index ikibz = 1 and ispin = 1 for this prototype (multi-k extension later).
  ...
  if (config.ISDF.isisdf)
    ...
    if isempty(id_vc) || isempty(id_vn)
    ...
    if ~exact_CH
      ...
      for iqbz = 1:nqbz
        ...
        if iqibz ~= iqibz_old
          ...
          if size(vn_data.helperqG, 1) ~= ng || size(nn_data.helperqG, 1) ~= ng
            warning('verifyW: inconsistent ng among vc/vn/nn helperqG, skip verification at iqibz=%d.', iqibz);
          else
            d_lat_data = lattice.manager('d_lat', 'get');
            ...
            for ikbz = 1:nkbz
              ...
              for iv = nrangev_row
                % Build all Mgvc(:, jc) first, then do a single BLAS-3 update.
            ...
            fprintf(['[verifyW] iqibz=%d ng=%d Nisdf(vc/vn/nn)=(%d,%d,%d)\n', ...
      ...
      for iqbz = 1:nqbz
        ...
        if iqibz ~= iqibz_old
          ...
          % Pick 1-2 imu channels, map helperqR_vc back to G with the same
          ...
          for iid = 1:numel(imu_check)
            ...
            if rel_tilde > fft_back_tol
              error('gw_cohsex_multi_k:helperqRBackToG', ...
            ...
            if exact_ch_debug
              ...
              fprintf(['[exact_CH debug] helperqR<->helperqG check iqibz=%d imu=%d ' ...
...
% 但是 0.5 的系数似乎是必须的
          ...
          for ikibz = 1:nkibz
            ...
            for indib = 1:nband
              ...
              switch lower(char(exact_ch_mode))
                ...
                otherwise
                  error('gw_cohsex_multi_k:exactCHMode', ...
              ...
              if exact_ch_debug && exact_ch_dbg_print_count < 12
                ...
                fprintf(['[exact_CH debug] mode=%s iqibz=%d ikibz=%d ib=%d ' ...
    ...
    % Dense no-ISDF prototype (single-k/single-q path for validation).
    if nkibz ~= 1 || nqbz ~= 1
