classdef R_lattice_m
  properties
    b1b2b3(3, 3) single = single(zeros(3, 3)) % columns are b1, b2, b3
    RL_vol(1, 1) single = single(0) % volume of the reciprocal lattice
    d3k_factor(1, 1) single = single(0) % prefactor for k-point integration: (2pi)^3 / RL_vol
    d3q_factor(1, 1) single = single(0) % prefactor for q-point integration: (2pi)^3 / RL_vol
    ...
    Ggrid_Cart(:, 3) single = single(zeros(0, 3)) % G-vectors in Cartesian coordinates
    ...
    tol = 1e-5 % tolerance for matching k-points in qindx
  end
  methods
