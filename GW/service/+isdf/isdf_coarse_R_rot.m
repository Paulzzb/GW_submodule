...
function [R_rot_coarse_wf, R_rot_coarse_apply] = isdf_coarse_R_rot(fftgrid_c)
% Symmetry index map on a regular coarse FFT RLU lattice (nr x nsym int32).
  ...
  for is = 1:nsym
    ...
    if norm(M2_r_RLU - round(M2_r_RLU)) > single(1e-4)
    ...
    R_rot_coarse_wf(:, is) = int32(round(i4));
