...
> 对外调用形式：**根目录**为 **`isdf.<文件名>`**；**子包**为 **`isdf.<子包>.<文件名>`**（见下）。
...
| 子目录 | MATLAB 前缀 | 内容 |
|--------|----------------|------|
| `+isdf/+coeff/` | `isdf.coeff.*` | 产生/使用 `coeff_seper` 的粗网格管线：`gen_coeff*`、`coeff_coarse_*`、`isdf_coarse_R_rot`、`divisors_int32`、`print_coarse_grid_report`、`isdf_get_coeff`、`isdf_apply_symm_on_coarse` 等 |
| `+isdf/+adaptive/` | `isdf.adaptive.*` | 自适应增心：`adaptiveisdf`、`adaptive_weight`、`isdf_schur_update`、`isdf_prod`、`run_adaptiveisdf`、`isdf_exclude_point`、`adaptiveisdf_kbz_rot_index` |
| `+isdf/+validation/` | `isdf.validation.*` | 效果验证：`isdf_validation`、`isdf_validate_HF` |
| `+isdf/+report/` | `isdf.report.*` | 文本报告：`gen_report` |
| `+isdf/` 根目录 | `isdf.*` | 池与门面：`manager`、`get`、`save2mod`、`free`、`driver`、`isdf_add`、`gen_tildeVq`、`isdf_get_nrange`、R 网格轨道辅助等（其余待你继续整理） |
| `+isdf/+base/` | `isdf.base.*` | 数据类 `isdf_m` |

---

## 1. 架构速览

- **对象池**：最多 `N_MAX=10` 个 `isdf.base.isdf_m` 实例，由 `manager` 持久化管理；`get` / `save2mod` / `free` / `isdf_add` 等是薄封装。
- **粗网格 ISDF 主路径**：`isdf.coeff.gen_coeff` → `gen_coeff_coarse` → `coeff_coarse_fft_grid_build` + `coeff_coarse_wf_extract` → 写回 `coeff_seper`；再 `isdf.gen_tildeVq`。
- **校验**：`isdf.validation.isdf_validation` 按 `interp_scheme` 分发；HF 细节在 `isdf.validation.isdf_validate_HF` + `isdf.report.gen_report`。
- **自适应**：`isdf.adaptive.adaptiveisdf` 依赖同包内 `isdf_schur_update` / `adaptive_weight` / `isdf_prod` 及根目录 `isdf.isdf_get_nrange` 等。
...
| 函数文件 | 功能 |
|----------|------|
| `manager` | 核心：池的 `get` / `save2mod` / `add` / `select` / `current` / `nmax` / `list` / `free`；校验 id 的局部函数 `isdf_manager_validate_id`。 |
| `get` | 读指定 id 或当前 id 的 `isdf_m`，不改变 current（除非省略 id 时用 current）。 |
| `save2mod` | 写回池；可指定 id。 |
| `free` | 清空整个池或单个 id。 |
| `isdf_add` | 分配第一个空槽，设 `desc`，设为 current，返回 id。 |
| `isdf_list` | 返回各槽的 `id` / `desc` / `assigned` / `empty` 结构体数组。 |
| `isdf_current` | 当前池 id（int32）。 |
| `isdf_nmax` | 池容量 `N_MAX`。 |
| `isdf_select` | 设置 current id（转调 `manager('select', …)`）。 |
...
| `isdf_driver` | **独立 input 目录**：读 `GWinput`/`config`，写 `ISDF_DB`；旧式一站式 ISDF 驱动。 |
...
| `gen_indices_coarse` | **遗留名**：注释称与 `gen_coeff_coarse` 的 r-grid模式相同；实现为调用 `isdf.gen_coeff_coarse()`（**无 id 参数，可能与当前 `gen_coeff_coarse(id)` 签名不一致**，重构时需核对）。 |
...
| 函数文件 | 功能 |
|----------|------|
| `gen_tildeVq` | 由粗 ISDF 与 Coulomb 构建 `tildeVq`（及 helper 量）；内部需 `isdf_get_nrange`。 |
...
| `isdf_get_nrange` | 按池 `desc`（`nn`/`vn`/`vc`）返回 ISDF 用的能带索引范围 `nrange1`/`nrange2`。 |
| `isdf_prod` | 四组波函数块上的 `(Psi*psi').*(Phi*phi')` 型乘积（用于 Gram/权重）。 |

**同文件内局部函数（`adaptiveisdf.m`）**
...
| `isdf_r_sampling_symm_orbit` | 在**当前 ISDF 的 `R_sampling_RLU`** 上构造同类轨道结构；可选 id，默认 current。 |

**同文件内局部函数（`isdf_r_sampling_symm_orbit.m`）**
...
| `+isdf/*.m` 顶层函数文件 | 40+ |
| `+isdf/+base/*.m` | 1（类） |
| 单文件内多个 `function`（局部/辅助） | `adaptiveisdf.m`、`isdf_validate_HF.m`、`isdf_r_sampling_symm_orbit.m`、`gen_report.m`、`gen_coeff.m`、`manager.m` 等 |

---

## 4. 命名与分类上的明显痛点（供你决断）

1. **`isdf_*` 前缀重复**：包名已是 `isdf`，函数名再 `isdf_get_nrange`、`isdf_validation` 等冗长。
2. **`gen_*` 混用**：有的表示路由（`gen_coeff`），有的表示具体算法（`gen_tildeVq`），有的是占位（`gen_coeff_kmeans`）。
3. **两套 driver**：`driver`（relay）、`isdf_driver`（旧目录流）、`run_adaptiveisdf`（自适应专用）职责交叉但入口不同。
