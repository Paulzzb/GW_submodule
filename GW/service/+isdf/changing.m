...
function out = changing(id)
  ...
  R_grid_bundle = fft_data.Rgrid_RLU(ind_bundle_points, :);
  ...
  R_rot_in_bundle = zeros(N_bundle, nsym);
  R_bundle2findgrid = zeros(N_bundle, nsym);
  for isym = 1:nsym
    rot_mtrx = symm_data.rot_mtrx_RLU_R(:, :, isym);
    M2_r_RLU = R_grid_bundle * rot_mtrx;
    ...
    R_rot_in_bundle(:, isym) = finegrid2bundle(indR);
  end


  % Get wavefunction on the bundle
  WF_bundle = zeros(N_bundle, wf_data.nb, wf_data.nk, wf_data.nspin);
  for ispin = 1:wf_data.nspin
    for ik = 1:wf_data.nk
      for ib = 1:wf_data.nb
        wf_t = wf_data.c(:, ib, ik, ispin);
        WF_bundle(:, ib, ik, ispin) = wf_t(bundle2finegrid);
      end
    end
  end

  out.N_bundle = N_bundle;
  out.R_grid_bundle = R_grid_bundle;
  ...
  for ispin = 1:wf_data.nspin
    for ikbz = 1:k_data.nbz
      for ib = 3:5
        ...
        if norm(wf_bz_in_bundle - wf_bz_in_bundle_dir) > 8e-5
