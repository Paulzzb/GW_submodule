...
- **`isdf.debug.init_from_config(config)`** is called from **`service_driver`** at startup, and from **`isdf.adaptive.run_adaptiveisdf`** after relay restore (paths that skip `service_driver`).
- After init, call sites use **`isdf.debug.on(tag)`** and **`isdf.debug.react(...)`** with **no `config` argument**.
- **`isdf.debug.clear()`** resets the cache; it is registered in **`service_reset_persistent`** alongside other ISDF resets.
...
Use **outside** expensive work (`if isdf.debug.on(tag) ... end`). Uses **only** the session cache. If `init_from_config` was never called, returns `false`.
...
- **`isdf.debug.on(config, tag)`** — read `config.ISDF` directly; does **not** update the cache.
- **`isdf.debug.level(config)`** / **`isdf.debug.level()`** — level from config or cache.
- **`isdf.debug.react(config, violation, msg [, tagForId])`**
- **`isdf.debug.check(tag, fh, msg [, id])`** or **`isdf.debug.check(config, tag, fh, msg [, id])`**

### Internal

- **`isdf.debug.cache`** — persistent store; not intended for general use.
...
Call **`isdf.debug.init_from_config(config)`** after you have the same `config` struct you would pass to `service_driver` (e.g. `load(..., ''config'')`).
