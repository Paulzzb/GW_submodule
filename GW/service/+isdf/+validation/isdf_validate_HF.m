        ...
        for iqbz = 1:nbz
          ...
          for ob = reshape(ob_list, 1, [])
            ...
            if strcmp(isdf_data.interp_scheme, "coarse")
              ...
              if ikrot_t ~= 1
                wf_1_c = isdf.coeff.isdf_apply_symm_on_coarse(id, wf_1_c, ikrot_t);
              end
              if isconj
                wf_1_c = conj(wf_1_c);
              end
            ...
            if strcmp(isdf_data.interp_scheme, "coarse")
              ...
              if ikprot_t ~= 1
                wf_2_c = isdf.coeff.isdf_apply_symm_on_coarse(id, wf_2_c, ikprot_t);
              end
              if isconj
                wf_2_c = conj(wf_2_c);
              end
            ...
            cc_rho = c_rho .* phase_shift_coarse;
            ...
            if norm(cc_rho - c_rho) > 1e-4
              error('The rho alpha is not correct');
            end
            ...
            % R_sampling_f = R_sampling(N_coarse+1:N_sampling, :);
            % fft_grid = single(fft_data.fftgrid);
            % SQ_R_sampling_f = R_sampling_f * symm_data.rot_mtrx_RLU_R(:, :, iqrot);
            % SQ_R_sampling_f = mod(round(SQ_R_sampling_f) + fft_grid, fft_grid);
            % ind_s2f = 1 + SQ_R_sampling_f(:, 1) + fft_grid(1) * SQ_R_sampling_f(:, 2) ...
            %            + fft_grid(1) * fft_grid(2) * SQ_R_sampling_f(:, 3);
            % ind_s2f = int32( round(ind_s2f) );
            % wf_1 = wave_functions.WF_apply_symm(isc);
            % wf_2 = wave_functions.WF_apply_symm(iscp);
            % rho_r = conj(wf_1) .* wf_2;
            % emm, I forgot phase_change part here in rho_r.
            % if norm(rho_r(ind_s2f) - cc_rho(N_coarse+1:N_sampling)) > 1e-4
            %   error('The rho r is not correct');
            % end
