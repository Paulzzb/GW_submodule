...
%
% Requires: isdf.gen_coeff_coarse(id) and isdf.gen_tildeVq(id) so that
...
% Optional outDir: directory for isdf_validate_HF_id<id>.txt (default pwd).
  ...
  Nmu = int32(size(isdf_data.coeff_seper, 1));
  if double(Nmu) ~= double(isdf_data.nisdf)
    warning('isdf:isdf_validation:NisdfSync', ...
      'coeff_seper rows (%d) vs isdf_data.nisdf (%d); validation uses coeff rows.', ...
      double(Nmu), double(isdf_data.nisdf));
  end
  ...
  if ndims(tildeVq) ~= 3
    error('isdf:isdf_validation:BadTildeVq', 'tildeVq must be 3-D (Nmu x Nmu x nibz) after squeeze.');
  end
  ...
  if strcmp(interp_scheme, "coarse")
    if isempty(R_rot_coarse) || size(R_rot_coarse, 1) ~= double(Nmu)
      error('isdf:isdf_validation:BadRrot', ...
        'R_rot_coarse missing or wrong size; expected %d rows.', double(Nmu));
    end
  end
  ...
  if size(R_sampling_RLU, 1) ~= double(Nmu)
    error('isdf:isdf_validation:BadRsampling', ...
      'R_sampling_RLU rows (%d) must match coeff_seper rows (%d).', ...
      size(R_sampling_RLU, 1), double(Nmu));
  end

  [Esum2, EsumISDF2, DiffEsum2] = isdf.isdf_validate_HF(id, outDir);
  % Text summary is written by isdf.gen_report (called inside isdf_validate_HF) to
