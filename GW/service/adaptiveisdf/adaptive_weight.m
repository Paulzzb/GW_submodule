...
%   Mirror of isdf_schur_update: init / update / get / clear. Implementation TBD.
%
  ...
  switch lower(mode)
    case 'init_update'
      ...
      for ib = 1:n_batches
        ...
        MrC1H = isdf_prod(Psi(batch_idx, :), Psi_on_grid(1:Nisdf, :), ...
                          Phi(batch_idx, :), Phi_on_grid(1:Nisdf, :)).';
        invL1MrC1H = single(invL_CCH(1:Nisdf, 1:Nisdf)) * single(MrC1H);
    ...
    case 'update'
      ...
      for ib = 1:n_batches
        ...
        if Nold == 0
          ...
          MrC2H = isdf_prod(Psi(batch_idx, :), Psi_on_grid(Nold+1:Nisdf, :), ...
                            Phi(batch_idx, :), Phi_on_grid(Nold+1:Nisdf, :)).';
          % invL2MrC2H = L_CCH(Nold+1:Nisdf, Nold+1:Nisdf) \ MrC2H;

          MrC1H = isdf_prod(Psi(batch_idx, :), Psi_on_grid(1:Nold, :), ...
    ...
    case 'init'
      ...
      w = zeros(Nw, 1);
      [nbrange1, nbrange2] = isdf_get_nrange(id);
      Psi = double(wf_data.c(:, nbrange1, 1, 1));
      Phi = double(wf_data.c(:, nbrange2, 1, 1));
      % Calculate diag(MMH), fill them into w
      for ic = 1:Nw
        Psi_r = Psi(ic, :);
        Phi_r = Phi(ic, :);
        w(ic) = isdf_prod(Psi_r, Psi_r, Phi_r, Phi_r);
      end
