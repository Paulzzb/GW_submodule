...
% helpful.
  ...
  % isdf_get_nrange is a normal function on path (adaptiveisdf/), not isdf.* package API.
  ...
  w = zeros(Nisdf_tmp, 1);
  for i = 1:Nisdf_tmp
    w(i) = isdf_prod(Psixga(i, :), Psixga(i, :), Phixga(i, :), Phixga(i, :));
  end

  well_ind = find(w > abs_tol);
  ill_ind = find(w <= abs_tol);
  if isempty(ill_ind)
    fprintf('isdf_exclude_point: id=%d — no centroids removed (all w > abs_tol=%.6e).\n', ...
      id, double(abs_tol));
    return
  end
  if isempty(well_ind)
  ...
  % save2mod(data, id) — not (id, data); do not assign to package name isdf.*
