...
%
%   run_adaptiveisdf('nn', output_dir)
%   run_adaptiveisdf('vn', output_dir)
%
% Inputs:
%   type       — 'nn' or 'vn' (case-insensitive). Selects coarse ISDF slot description and nisdf rule.
%   output_dir — Directory where isdf_validate_HF writes the HF report via isdf.gen_report
...
%   test_relay_stage.mat (e.g. test_Si), then call run_adaptiveisdf(...).
%
...
% validation uses that directory as pwd for report generation.
  ...
  adf_dir = fileparts(this_file);
  service_dir = fileparts(adf_dir);
  gw_root = fileparts(service_dir);
  ...
  isdf.gen_coeff(cfg, id_slot);
  isdf.print_coarse_grid_report(id_slot);
