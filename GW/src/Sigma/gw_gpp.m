function gw_gpp(GWinfo, options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for igcol = 1:ng
  Omega2 =  wpeff(GWinfo, options, igcol); % There should not be 1/vol here...
...
for ibandouter = nv-nv_ener+1 : nv+nc_ener % line 1167 of mtxel_cor
  ...
  for ibandinner = nv-nv_oper+1 : nv+nc_oper % line 1214 of mtxel_cor
    ...
    for igcol = 1:ng % line 1473, do my_igp = 1, ngpown
      ...
      if (flagocc == true)
        asxtemp = asxtemp - ssx_array * occ * dcoul(igcol) / GWinfo.vol;
      end
      achtemp = achtemp + sch_array * dcoul(igcol) / GWinfo.vol;
    end % for igcol = 1:ng
...
%   if aqsFlag
%     Mgvn = GWinfo.aqs{ioper}(:, nv-nv_ener+1:nv+nc_ener);
%   else
%     Mgvn = mtxel_sigma(ioper, GWinfo, options.Groundstate, (nv-nv_ener+1:nv+nc_ener));
%   end
%   Mgvn = conj(Mgvn);
%   Ex = Ex + Mgvn' * Dcoul * Mgvn / GWinfo.vol;
% end
