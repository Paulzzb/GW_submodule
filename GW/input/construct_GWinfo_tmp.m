function GWinfo = construct_GWinfo_tmp(GWinfo)
  ...
  a1a2a3 = single( GWinfo.supercell' );
  b1b2b3 = single( 2*pi * inv(a1a2a3)');
  ...
  Ggrid_Cart = single(Ggrid_RLU) * b1b2b3';
  %
  Rgrid_RLU = single( zeros(nr, 3) );
  count = 0;
  ...
  for i = 1:nsym
    rot_mtrx_RLU_G{i} = single( rot_mtrx_RLU_G{i} );
  end
  ...
  for irot = 1:nsym
    if ~isempty(GWinfo.mapping.g_rot{irot})
      perm1 = single( round( GWinfo.mapping.g_rot{irot} ) );
      perm2 = single( round( G_rot(:, irot) ) );
