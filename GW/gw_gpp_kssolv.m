function  eqp = gw_gpp(ksinfo, options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   ksinfo_bgw = setksinfo(ksinfo, options, './');
% end
...
if (options.isisdf)
  ...
  inveps = 4 * Mgvc * (diag(Eden) * Mgvc') / (ksinfo.vol);
...
% 	end
  ...
  for igcol = 1:ng
    Omega2 =  wpeff(ksinfo, igcol, qindex, options.setting_method); % There should not be 1/vol here...
%     if (exist('wpmtx', 'file') == 2)
...
% 				  	end % if (schttflag == true)
        ...
        if (flagocc == true)
          asxtemp = asxtemp - ssx_array * occ * vcoul(igcol) / ksinfo.vol;
        end
        achtemp = achtemp + sch_array * vcoul(igcol) / ksinfo.vol;
      end % for igcol = 1:ng
...
function Omega2 = wpeff(ksinfo, igcol, qindex, setting_method)
