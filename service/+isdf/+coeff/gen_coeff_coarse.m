...
%
%   [Nmu, ind_mu] = isdf.coeff.gen_coeff_coarse()
%       R-grid subset selection (gcd / step); Nmu_tmp = isdf_ratio * nrep.
%
%   [fftgrid_i, fft_sz, nfft_d, fftgrid_c, Nmu, R_coarse_RLU, R_rot_coarse] = ...
%       isdf.coeff.gen_coeff_coarse('fft_grid')
%       Regular coarse FFT box; Nmu_tmp = isdf_ratio * nb.
%
%   wf_on_coarse = isdf.coeff.gen_coeff_coarse('wf', fftgrid_i, fft_sz, fftgrid_c, Nmu, R_coarse_RLU)
%       Map wavefunctions to coarse RLU sites.

  fft_data = FFT.get();

  isdf.coeff.coeff_coarse_fft_grid_build(id);
  ...
  wf_on_coarse = isdf.coeff.coeff_coarse_wf_extract(fftgrid_c, R_coarse_RLU);
  [wf_on_coarse, R_coarse_RLU, R_rot_coarse] = isdf.coeff.coeff_coarse_wf_filter( ...
