function idnew = adaptiveisdf(id, cfg_isdf)
  ...
  % isdf.adaptive.isdf_exclude_point(id);
  ...
  isdf.adaptive.adaptive_weight('init', id);
  [Psixga, Phixga] = isdf.adaptive.adaptive_weight('get_wf_xga', R_sampling_indices);

  isdf.adaptive.isdf_schur_update('init', Nisdf, Psixga, Phixga, ...
  ...
  w = isdf.adaptive.adaptive_weight('get');
  loss0 = sum(w);
  fprintf('Initial loss: %f\n', loss0);
  isdf.adaptive.adaptive_weight('init_update');
  w = isdf.adaptive.adaptive_weight('get');
  loss = sum(w);
  ...
  while Nisdf_new < Nisdf_new_max && loss0 > 0 && (loss / loss0 > threshold) && ~isempty(global_index_remain)
    ...
    for iadd = 1:num_add_i
      ...
      while Nremain > 0 && ~schur_ok
        ...
        w_candidate(1:Nremain) = isdf.adaptive.adaptive_weight('get', candidate_indices(1:Nremain));
        ...
        [Psi_on_new_grid, Phi_on_new_grid] = ...
          isdf.adaptive.adaptive_weight('get_wf_xga', selected_indices(iadd));
        schur_ok = isdf.adaptive.isdf_schur_update('update', 1, Psi_on_new_grid, Phi_on_new_grid, selected_indices(iadd));
        if ~schur_ok
          if loss0 > 0
            rel_now = loss / loss0;
          else
    ...
    isdf.adaptive.adaptive_weight('update', tocal, n_selected_iter);
    % 4. Update the loss
    w(global_index_remain) = isdf.adaptive.adaptive_weight('get', global_index_remain);
  ...
  %   [Psi_on_new_grid, Phi_on_new_grid] = isdf.adaptive.adaptive_weight('get_wf_xga', ind_new);
