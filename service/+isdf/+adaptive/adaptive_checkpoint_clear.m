...
%
%   isdf.adaptive.adaptive_checkpoint_clear(coarse_id)
%   isdf.adaptive.adaptive_checkpoint_clear(coarse_id, desc)
%
% Deletes desc-keyed checkpoint and legacy id-only file (if present).
  ...
  paths = { ...
    isdf.adaptive.adaptive_checkpoint_path(coarse_id, desc), ...
    isdf.adaptive.adaptive_checkpoint_legacy_path(coarse_id) ...
  };
  for k = 1:numel(paths)
