...
%   cols = isdf.adaptive_single.MrC1H('cached_cols', row_idx)
%   isdf.adaptive_single.MrC1H('set_range', row_idx, cstart, vals)
%   blk = isdf.adaptive_single.MrC1H('get', row_idx, cstart, cend)
%   isdf.adaptive_single.MrC1H('set_cached_cols', row_idx, ncols)
  ...
  switch lower(mode)
    case 'ensure'
      ...
      if isempty(cache) || size(cache, 1) ~= Nw || size(cache, 2) ~= Nisdfmax
        cache = complex(zeros(Nw, Nisdfmax, 'single'));
