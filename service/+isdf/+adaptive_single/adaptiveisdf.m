...
function idnew = adaptiveisdf(id, cfg_isdf)
  ...
  [ck_loaded, idnew] = isdf.adaptive_single.adaptive_checkpoint_try_load(id);
  if ck_loaded
    return;
  end
  ...
  if ~isempty(import_root)
    [ck_loaded, idnew] = isdf.adaptive_single.adaptive_checkpoint_import_from_isdf(id, import_root);
    if ck_loaded
      return;
    end
  end
  ...
  % isdf.adaptive_single.isdf_exclude_point(id);
  ...
  adaptive_backend = 'adaptive_single';
  adaptive_arithmetic = 'single isdf.prod, double Schur algebra';
  adaptiveisdf_print_run_config(id, isdf_data, params, threshold, num_add, ratio, ...
    adaptive_backend, adaptive_arithmetic);
  isdf.adaptive_single.adaptive_weight('set_batch_size', params.weight_batch_size);
  ...
  isdf.adaptive_single.adaptive_weight('init', id);
  [Psixga, Phixga] = isdf.adaptive_single.adaptive_weight('get_wf_xga', R_sampling_indices);

  isdf.adaptive_single.isdf_schur_update('init', Nisdf, Psixga, Phixga, ...
  ...
  w = isdf.adaptive_single.adaptive_weight('get');
  loss0 = sum(w);
  fprintf('Initial loss: %f\n', loss0);
  isdf.adaptive_single.adaptive_weight('init_update');
  w = isdf.adaptive_single.adaptive_weight('get');
  loss = sum(w);
  ...
  while Nisdf_new < Nisdf_new_max && loss0 > 0 && (loss / loss0 > threshold) && ~isempty(global_index_remain)
    ...
    for iadd = 1:num_add_i
      ...
      while Nremain > 0 && ~schur_ok
        ...
        w_candidate(1:Nremain) = isdf.adaptive_single.adaptive_weight('get', candidate_indices(1:Nremain));
        ...
        [Psi_on_new_grid, Phi_on_new_grid] = ...
          isdf.adaptive_single.adaptive_weight('get_wf_xga', selected_indices(iadd));
        schur_ok = isdf.adaptive_single.isdf_schur_update('update', 1, Psi_on_new_grid, Phi_on_new_grid, selected_indices(iadd));
        if ~schur_ok
          if loss0 > 0
            rel_now = loss / loss0;
          else
        ...
        isdf.adaptive_single.adaptive_weight('update', candidate_indices(1:Nremain), 1);
        Nremain = Nremain - 1;
    ...
    isdf.adaptive_single.adaptive_weight('update', tocal, n_selected_iter);
    % 4. Update the loss
    w(global_index_remain) = isdf.adaptive_single.adaptive_weight('get', global_index_remain);
  ...
  [Nisdf_new, ~, ~, ~, Psi_on_grid, Phi_on_grid] = ...
         isdf.adaptive_single.isdf_schur_update('get');
  
  Nextra = Nisdf_new - Nisdf;
  % Check if the new-added points correcly indiced.
  % for i = 1:Nextra
  %   ind_new = isdf_new_indices(i);
  %   [Psi_on_new_grid, Phi_on_new_grid] = isdf.adaptive_single.adaptive_weight('get_wf_xga', ind_new);
  ...
  isdf.adaptive_single.adaptive_checkpoint_save(id, idnew);
