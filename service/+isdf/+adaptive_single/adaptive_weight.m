  ...
  switch lower(mode)
    ...
    case 'init_update'
      ...
      for ib = 1:n_batches
        ...
        cols_have = isdf.adaptive_single.MrC1H('cached_cols', batch_idx_col);
        ...
        if any(need_mask)
          ...
          for is = 1:length(starts)
            ...
            isdf.adaptive_single.MrC1H('set_range', rows_now, cstart, MrC1H_new);
            isdf.adaptive_single.MrC1H('set_cached_cols', rows_now, Nisdf);
            cols_have(row_mask) = Nisdf;
          end
        end
        MrC1H = isdf.adaptive_single.MrC1H('get', batch_idx, 1, Nisdf);
        invL1MrC1H = invL_CCH(1:Nisdf, 1:Nisdf) * MrC1H';
        w(batch_idx_col) = w(batch_idx_col) - single(sum(abs(invL1MrC1H).^2, 1).');
      end
    ...
    case 'update'
      ...
      for ib = 1:n_batches
        ...
        if Nold == 0
          ...
          isdf.adaptive_single.MrC1H('set_range', batch_idx, 1, MrC1H);
          isdf.adaptive_single.MrC1H('set_cached_cols', batch_idx_col, Nisdf);
          ...
          if any(need_mask)
            ...
            for is = 1:length(starts)
              ...
              isdf.adaptive_single.MrC1H('set_range', rows_now, cstart, MrC1H_old);
              isdf.adaptive_single.MrC1H('set_cached_cols', rows_now, Nold);
          ...
          isdf.adaptive_single.MrC1H('set_range', batch_idx, Nold + 1, MrC2H);
          isdf.adaptive_single.MrC1H('set_cached_cols', batch_idx_col, Nisdf);
          % invL2MrC2H = L_CCH(Nold+1:Nisdf, Nold+1:Nisdf) \ MrC2H;

          MrC1H = isdf.adaptive_single.MrC1H('get', batch_idx, 1, Nold);
          ...
          % invL2MrC2H = single(invL_CCH(Nold+1:Nisdf, Nold+1:Nisdf)) * single(MrC2H);
          % invL21MrC1H = single(invL_CCH(Nold+1:Nisdf, 1:Nold)) * single(MrC1H);
          % wtilde2 = single(sum(abs(invL2MrC2H).^2, 1));
          % wtilde2 = wtilde2 + single(2 * real(sum(conj(invL21MrC1H) .* invL2MrC2H, 1)));
          % wtilde2 = wtilde2 + single(sum(abs(invL21MrC1H).^2, 1));
          ...
          w(batch_idx_col) = w(batch_idx_col) - single(wtilde3.');
          if any(wtilde3 < 0)
    ...
    case 'init'
      ...
      Psi = zeros(Nw, length(nbrange1)*k_data.nbz, 'single');
      Phi = zeros(Nw, length(nbrange2)*k_data.nbz, 'single');
      ...
      for ikbz = 1:k_data.nbz
        ...
        for ib1 = 1:numel(nbrange1)
          ...
          Psi(:, count1) = single(wave_functions.WF_apply_symm(param1));
          count1 = count1 + 1;
        end
        for ib2 = 1:numel(nbrange2)
          param2 = [nbrange2(ib2), ikibz, ikrot, ispin];
          Phi(:, count2) = single(wave_functions.WF_apply_symm(param2));
          count2 = count2 + 1;
        end
      end

      isdf.adaptive_single.MrC1H('clear');
