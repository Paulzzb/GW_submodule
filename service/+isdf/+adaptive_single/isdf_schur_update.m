...
%   CCH / L_CCH / invL_CCH and subsequent linear algebra use double.
  ...
  switch lower(mode)
    case 'update'
      ...
      Psi_on_new_grid = single(varargin{2});
      Phi_on_new_grid = single(varargin{3});
      selected_global_idx = [];
      if nargin >= 5
      ...
      if Nadd == 1
        if oldNisdf > 0
          if isempty(has_rank1_prod_mex)
            has_rank1_prod_mex = ~isempty(which('isdf.adaptive_single.isdf_schur_rank1_prod_mex'));
          end
          ...
          if ~isempty(selected_global_idx)
            ...
            isdf.adaptive_single.MrC1H('ensure', double(wf_data.nc), Nisdfmax);
            cols_have = isdf.adaptive_single.MrC1H('cached_cols', selected_global_idx);
            if cols_have < oldNisdf
              ...
              isdf.adaptive_single.MrC1H('set_range', selected_global_idx, cstart, C2C1H_new);
              isdf.adaptive_single.MrC1H('set_cached_cols', selected_global_idx, oldNisdf);
            end
            C2C1H = isdf.adaptive_single.MrC1H('get', selected_global_idx, 1, oldNisdf);
          else
          ...
          if isempty(has_rank1_mex)
            has_rank1_mex = ~isempty(which('isdf.adaptive_single.isdf_schur_rank1_mex'));
          end
          if has_rank1_mex
            try
              [L21_row, L22_mex, invL_new_old] = ...
        ...
        if oldNisdf == 0
          ...
          C2C1H = double(isdf.prod(Psi_on_new_grid, Psi_on_grid(oldIdx, :), Phi_on_new_grid, Phi_on_grid(oldIdx, :)));
        end
        if oldNisdf > 0
          invL11 = invL_CCH(oldIdx, oldIdx);
          L21 = C2C1H * invL11';
    ...
    case 'init'
      ...
      Psi_on_grid(1:Nisdf, :) = single(varargin{2});
      Phi_on_grid(1:Nisdf, :) = single(varargin{3});

      CCH = zeros(Nisdfmax, Nisdfmax, 'double');
      L_CCH = zeros(Nisdfmax, Nisdfmax, 'double');
      invL_CCH = zeros(Nisdfmax, Nisdfmax, 'double');
      CCH(1:Nisdf, 1:Nisdf) = double(isdf.prod(Psi_on_grid(1:Nisdf, :), Psi_on_grid(1:Nisdf, :), ...
