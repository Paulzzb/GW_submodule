...
function gen_bundle(id)
  ...
  scheme = isdf_data.interp_scheme;
  if scheme == "adaptive" || scheme == "adaptive_sc" || scheme == "coarse_sc"
  ...
  if N_old > 0
    if ~isempty(isdf_data.tmp) && numel(isdf_data.tmp) > 0
      WF_bundle(1:N_old, :, :, :) = isdf_data.tmp(:, :, :, :);
    elseif ~isempty(isdf_data.coeff_seper) && size(isdf_data.coeff_seper, 1) >= N_old
      WF_bundle(1:N_old, :, :, :) = isdf_data.coeff_seper(1:N_old, :, :, :);
    else
      error('isdf.rsymm:gen_bundle:CoarseWf', ...
        'ISDF id=%d: coarse WF missing (tmp and coeff_seper empty).', int32(id));
    end
  end
  ...
  isdf_data.bundle_struct.N_sampling = int32(N_old + N_new);
  ...
  if scheme == "adaptive_sc" || scheme == "supercell" || scheme == "coarse_sc"
    return;
  end
