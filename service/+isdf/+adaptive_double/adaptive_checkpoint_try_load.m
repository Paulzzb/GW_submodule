...
%
%   [loaded, idnew] = isdf.adaptive_double.adaptive_checkpoint_try_load(coarse_id)
%
...
% are used only when isdf_desc in the file matches the current coarse slot.
  ...
  fpath = isdf.adaptive_double.adaptive_checkpoint_path(coarse_id, expected_desc);
  if exist(fpath, 'file') ~= 2
    legacy = isdf.adaptive_double.adaptive_checkpoint_legacy_path(coarse_id);
