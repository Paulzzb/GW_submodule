...
%
% Precision (this package: +adaptive_double):
...
%
  ...
  switch lower(mode)
    case 'update'
      ...
      if Nadd == 1
        % First, we do ISDF_prod to calculate 
        ...
        if oldNisdf > 0
          ...
          % Use mex to do ISDF_prod if available.
          if has_rank1_prod_mex
            try
              ...
              if isfinite(L22_mex) && real(L22_mex) > 0 && ...
                ...
                % Copy the results to the persistent variables.
          ...
          % No fast path, use MATLAB to do ISDF_prod.
          ...
          % Use mex to do cholesky factorization if available.
