...
static double get_scalar_as_double(const mxArray *a) {
  if (!mxIsNumeric(a) || mxIsComplex(a) || mxIsEmpty(a)) {
    mexErrMsgIdAndTxt("isdf_schur_rank1_mex:Type", "Scalar input must be real numeric.");
  }
  if (mxIsDouble(a)) {
    return mxGetDoubles(a)[0];
  }
  return mxGetScalar(a);
}

static void copy_vector_to_double(const mxArray *a, double *out, mwSize n) {
  mwSize m = mxGetM(a);
  mwSize k = mxGetN(a);
  mwSize len = m * k;
  mwSize i;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (!mxIsDouble(invL_in) || mxIsComplex(invL_in) || mxGetNumberOfDimensions(invL_in) != 2) {
    mexErrMsgIdAndTxt("isdf_schur_rank1_mex:Type", "invL_CCH must be a real double 2D matrix.");
  }
  ...
  plhs[0] = mxCreateNumericMatrix(1, oldN, mxDOUBLE_CLASS, mxREAL); /* L21_row */
  plhs[1] = mxCreateNumericMatrix(1, 1, mxDOUBLE_CLASS, mxREAL);    /* L22 */
  plhs[2] = mxCreateNumericMatrix(1, oldN, mxDOUBLE_CLASS, mxREAL); /* invL_new_old */
  ...
  copy_vector_to_double(c2c1h_in, c, oldN);
  c2c2h = get_scalar_as_double(c2c2h_in);
