...
function bundle_refresh(id, N_new, indices_new, idnew)
  ...
  %
  %   Optional WF consistency checks: isdftest.debug.init_from_config (see ISDF_debug.md),
  ...
  if idnew ~= id
    list = isdftest.isdftest_list();
    if list(idnew).empty
      isdf_dup = isdf_data;
      isdf_dup.id = idnew;
      isdftest.save2mod(isdf_dup, idnew);
    end
    isdf_data = isdftest.get(idnew);
  end
  isdf_data1 = isdftest.get(id);
  fft_data = FFT.get();
  ...
  for inew = 1:N_new
    r_new = double( fft_data.Rgrid_RLU(indices_new(inew), :) );
    tmp = find(all( abs( Rgrid_b_new(1:Nb_new, :) - r_new ) < rot_tol, 2));
    if length(tmp) > 2
      error('isdftest.rsymm:bundle_refresh:rNewNotInBundle', sprintf( ...
        'R_sampling_RLU row %d is not any row of bundle_struct.R_grid_bundle (double, ==). r_new = [%g %g %g].', ...
    ...
    if isempty(tmp)
      ...
      if round(ind_rot_finegrid) - ind_rot_finegrid > rot_tol
        error('isdftest.rsymm:bundle_refresh:indRotFinegridNotInteger', sprintf( ...
  ...
  for isym = 1:nsym
    ...
    % Use double(R) to avoid false rejects from double*double roundoff; tolerance 1e-3 matches FFT/driver.
    ...
    if max(res_int(:)) > rot_tol
      error('isdftest.rsymm:bundle_refresh:RotInBundleNotInteger', sprintf( ...
  ...
  if Nb_not_in_finegrid > 0
    for isym = 1:nsym
      ...
      if ~isempty(iz)
        ...
        fprintf(1, ['[isdftest.rsymm.bundle_refresh] isym=%d: tmp rows with all 3 comps |.|<%g ', ...
  ...
  if isdftest.debug.on('rsymm/bundle_refresh')
    ...
    for isym = 1:nsym
      ...
      for irb = ind_b_in_finegrid(1:Nb_in_finegrid)
        ...
        rbg_rot_tgt = double(Rgrid_b_new(irbrot, :));
        ...
        isdftest.debug.react(dmx > rot_tol, msg, 'bundle_refresh_rot_v0');
      end
    end
    ...
    for isym = 1:nsym
      ...
      for k = 1:Nb_in_finegrid
        ...
        isdftest.debug.react(bad_idx, msg_idx, 'bundle_refresh_rot_idx');
        if bad_idx
          continue
        end
        ...
        isdftest.debug.react(dgeom > rot_tol, msg_geom, 'bundle_refresh_rot_geom');
      end
    end
    ...
    for ispin = 1:nspin
      for ikbz = 1:nbz
        ...
        for ib = 1:nb
          ...
          isdftest.debug.react(norm(wf_in_bundle - wf_ikbz_b(ind_b_in_finegrid(1:Nb_in_finegrid))) > 8e-5, ...
          ...
          isdftest.debug.react(norm(wf_sampling1 - wf_sampling2) > 8e-5, ...
            'The wavefunction on the sampling is not correct', 'bundle_refresh_sampling12');
          isdftest.debug.react(norm(wf_sampling1 - wf_sampling3) > 8e-5, ...
    ...
    for ispin = 1:nspin
      for ikbz = 1:nbz
        ...
        for ib = 3:5
          ...
          isdftest.debug.react(norm(wf_sampling - wf_Srotk_R_sampling) > 8e-5, ...
  ...
  %
  isdftest.save2mod(isdf_data, idnew);
    ...
    for ispin = 1:wf_data.nspin
      for ikbz = 1:k_data.nbz
        for ib = 3:5
          ...
          isdftest.debug.react(nmf > 8e-5, msg_wf, 'bundle_refresh_postsave_wf_dir');
          for iq = 1:k_data.nbz
            ...
            isdftest.debug.react(nrint > 5e-4, msg_rot, 'bundle_refresh_postsave_sampling_rot');
            Sq_R_grid_sampling = double(round(Sq_R_grid_sampling));
            ...
            isdftest.debug.react(nmfine > 8e-5, msg_fine, 'bundle_refresh_postsave_Sq_fine');
            wf_ikibz_coarse = isdf_data1.coeff_seper(:, ib, ikibz, ispin);
            wf_ikibz_coarse = wf_ikibz_coarse(1:N_coarse);
            wf_ikbz_coarse = isdftest.coeff.isdf_apply_symm_on_coarse(id, wf_ikibz_coarse, ikrot);
            ...
            isdftest.debug.react(nmcoarse > 8e-5, msg_coarse, 'bundle_refresh_postsave_Sq_coarse');
          end
        end
      end
    end
