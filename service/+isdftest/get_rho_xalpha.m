...
% Downstream contraction is c_t' * tildeVq * c_t.
  ...
  if ~isfield(param, 'qs') || numel(param.qs) < 2
    error('isdftest:get_rho_xalpha:qs', 'param.qs must be [iGo, iqibz, iqrot].');
  end
  ...
  u_xalpha1 = isdftest.get_u_xalpha(id, param.is, iqrot);
  u_xalpha2 = isdftest.get_u_xalpha(id, param.os, iqrot);
  ...
  if ~has_cell_factors
    error('isdftest:get_rho_xalpha:MissingTruncFactors', ...
      ['CCHq_trunc_factors{%d} missing for id=%d; run isdftest.gen_tildeVq first ', ...
  ...
  if ~isfield(fac, 'V_trunc') || ~isfield(fac, 'Lambda_trunc')
    error('isdftest:get_rho_xalpha:BadFactors', ...
      'CCHq_trunc_factors{%d} missing V_trunc/Lambda_trunc.', iqibz);
  end
  isdftest.prod_C_inv_t('set_factors', fac.V_trunc, fac.Lambda_trunc, isdf_data.svd_ratio);
  rho_xalpha = isdftest.prod_C_inv_t('prod', 'l', c(:));
end
