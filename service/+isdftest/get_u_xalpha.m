function u_xalpha = get_u_xalpha(id, isc, iqrot)
  ...
  % Optional consistency checks under isdftest.debug switches.
  % Cache isdftest.debug.on('u_xalpha_sampling'): hot path calls this millions of times;
  % refresh on first use after get_u_xalpha('reset') / service_reset_persistent.
  if isempty(u_xalpha_sampling_debug_on)
    u_xalpha_sampling_debug_on = isdftest.debug.on('u_xalpha_sampling');
  end
  if u_xalpha_sampling_debug_on
    ...
    isdftest.debug.check('u_xalpha_sampling', ...
  ...
  % u_ikbz_c = isdftest.coeff.isdf_apply_symm_on_coarse(id, u_ikibz_c, ikrot);
  ...
  % end
  % Sq_R_f_sampling = double( round(Sq_R_f_sampling) );
