...
%
% Text report (band summary, preview, stats, global sums) is written by isdftest.report.gen_report to
...
% tildeVq, wf_on_coarse, R_rot_coarse, varargin
        ...
        for iqbz = 1:nbz
          ...
          for ob = 1:obnb
            ...
            if strcmp(isdf_data.interp_scheme, "coarse")
              ...
              if ikrot ~= 1
                wf_1_c = isdftest.coeff.isdf_apply_symm_on_coarse(id, wf_1_c, ikrot);
              end
              % apply rotation of iqrot
              % u_{ib, ikbz}(S_iqrot * r)
              if iqrot ~= 1
                % wf_1_c = isdftest.coeff.isdf_apply_symm_on_coarse(id, wf_1_c, iqrot);
            ...
            if strcmp(isdf_data.interp_scheme, "coarse")
              ...
              if ikprot ~= 1
                wf_2_c = isdftest.coeff.isdf_apply_symm_on_coarse(id, wf_2_c, ikprot);
              end
              if iqrot ~= 1
                ind = isdf_data.R_rot_extra(1:N_coarse, iqrot);
                wf_2_c = wf_2_c(ind);
            ...
            % c_rho = isdftest.coeff.isdf_get_coeff(id, wf_1_c, wf_2_c, ikrot, ikprot);
            c_rho = conj(wf_1_c) .* wf_2_c;
            
            Go = double(r_lat_data.Ggrid_RLU(iGo, :));
            % SqR_scal = double(R_sampling_RLU*symm_data.rot_mtrx_RLU_R(:, :, iqrot)) ./ double(fft_data.fftgrid);
  ...
  fpath = isdftest.report.gen_report(report, outDir);
