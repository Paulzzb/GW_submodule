...
%
% Requires: isdftest.coeff.gen_coeff_coarse(id) and isdftest.gen_tildeVq(id) so that
%   interp_scheme == "coarse", coeff_seper holds wf_on_coarse, tildeVq is filled.
%
% Optional id: omit to use isdftest.isdftest_current().
% Optional outDir: directory for isdf_validate_HF_id<id>.txt (default pwd).
  ...
  if isempty(isdf_data.tildeVq)
    error('isdf:isdf_validation:NoTildeVq', ...
      'tildeVq is empty; run isdftest.gen_tildeVq(id) first. id=%d', id);
  end
  ...
  [Esum2, EsumISDF2, DiffEsum2] = isdftest.validation.isdf_validate_HF(id, outDir);
  % Text summary is written by isdftest.report.gen_report (called inside isdf_validate_HF) to
