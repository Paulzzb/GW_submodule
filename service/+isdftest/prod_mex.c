...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs != 4) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:nrhs", "Expected 4 inputs: Psi, psi, Phi, phi.");
  }
  if (nlhs > 1) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:nlhs", "Expected one output.");
  }
  ...
  if (!mxIsDouble(Psi) || !mxIsDouble(psi) || !mxIsDouble(Phi) || !mxIsDouble(phi)) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:type", "prod_mex supports double real/complex inputs only.");
  }
  ...
  n = mxGetM(psi);
  if (mxGetN(psi) != k1) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:size", "size(Psi,2) must equal size(psi,2).");
  }
  if (mxGetM(Phi) != m) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:size", "size(Psi,1) must equal size(Phi,1).");
  }
  if (mxGetM(phi) != n) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:size", "size(psi,1) must equal size(phi,1).");
  }
  k2 = mxGetN(Phi);
  if (mxGetN(phi) != k2) {
    mexErrMsgIdAndTxt("isdftest:prod_mex:size", "size(Phi,2) must equal size(phi,2).");
  }
