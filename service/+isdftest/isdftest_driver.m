...
%
function isdftest_driver(input_dir)
  % isdftest_driver - driver for implementing ISDF\
  % Results are saved in ISDF_DB in input_dir
  % -----------------------------------------------------------------
  cleanup = QPlog_push('isdftest_driver');
  ...
  if isempty(GWinfo.psir) && config.isdftest.isisdf
