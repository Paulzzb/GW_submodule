function [N_r_orbit, Nr, Rgrid_RLU, Rgrid_rep_RLU, ir2rep, ir2rot] = isdftest_r_sampling_symm_orbit(id)
%isdftest_r_sampling_symm_orbit Build symmetry-orbit mapping on ISDF sampling points.
%
% Usage:
%   isdftest_r_sampling_symm_orbit()      % use current ISDF id in manager
%   isdftest_r_sampling_symm_orbit(id)    % use explicit ISDF id
%
% Output convention:
...
%   Rgrid_RLU(ir, :) = Rgrid_rep_RLU(ir2rep(ir), :) * rot_mtrx_RLU_R(:, :, ir2rot(ir))
  ...
  if Nr <= 0
    error('isdftest_r_sampling_symm_orbit:EmptySampling', 'isdf_data.R_sampling_RLU is empty.');
  end
  if nsym <= 0
    error('isdftest_r_sampling_symm_orbit:NoSymmetry', 'symmetry.rot_mtrx_RLU_R is empty.');
  end
  ...
  if any(ir2rep == 0) || any(ir2rot == 0)
    error('isdftest_r_sampling_symm_orbit:IncompleteCover', ...
  ...
  for ir = 1:Nr
    ...
    if local_periodic_dist(r_rhs, r_lhs, fftgrid_d) > tol
      error('isdftest_r_sampling_symm_orbit:RelationMismatch', ...
