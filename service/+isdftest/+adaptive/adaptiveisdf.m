function idnew = adaptiveisdf(id, cfg_isdf)
  ...
  [ck_loaded, idnew] = isdftest.adaptive.adaptive_checkpoint_try_load(id);
  if ck_loaded
    return;
  end
  ...
  if ~isempty(import_root)
    [ck_loaded, idnew] = isdftest.adaptive.adaptive_checkpoint_import_from_isdf(id, import_root);
    if ck_loaded
      return;
    end
  end
  ...
  % isdftest.adaptive.isdf_exclude_point(id);
  ...
  isdftest.adaptive.adaptive_weight('set_batch_size', params.weight_batch_size);
  ...
  isdftest.adaptive.adaptive_weight('init', id);
  [Psixga, Phixga] = isdftest.adaptive.adaptive_weight('get_wf_xga', R_sampling_indices);

  isdftest.adaptive.isdf_schur_update('init', Nisdf, Psixga, Phixga, ...
  ...
  w = isdftest.adaptive.adaptive_weight('get');
  loss0 = sum(w);
  fprintf('Initial loss: %f\n', loss0);
  isdftest.adaptive.adaptive_weight('init_update');
  w = isdftest.adaptive.adaptive_weight('get');
  loss = sum(w);
  ...
  while Nisdf_new < Nisdf_new_max && loss0 > 0 && (loss / loss0 > threshold) && ~isempty(global_index_remain)
    ...
    for iadd = 1:num_add_i
      ...
      while Nremain > 0 && ~schur_ok
        ...
        w_candidate(1:Nremain) = isdftest.adaptive.adaptive_weight('get', candidate_indices(1:Nremain));
        ...
        [Psi_on_new_grid, Phi_on_new_grid] = ...
          isdftest.adaptive.adaptive_weight('get_wf_xga', selected_indices(iadd));
        schur_ok = isdftest.adaptive.isdf_schur_update('update', 1, Psi_on_new_grid, Phi_on_new_grid, selected_indices(iadd));
        if ~schur_ok
          if loss0 > 0
            rel_now = loss / loss0;
          else
        ...
        isdftest.adaptive.adaptive_weight('update', candidate_indices(1:Nremain), 1);
        Nremain = Nremain - 1;
    ...
    isdftest.adaptive.adaptive_weight('update', tocal, n_selected_iter);
    % 4. Update the loss
    w(global_index_remain) = isdftest.adaptive.adaptive_weight('get', global_index_remain);
  ...
  [Nisdf_new, ~, ~, ~, Psi_on_grid, Phi_on_grid] = ...
         isdftest.adaptive.isdf_schur_update('get');
  
  Nextra = Nisdf_new - Nisdf;
  % Check if the new-added points correcly indiced.
  % for i = 1:Nextra
  %   ind_new = isdf_new_indices(i);
  %   [Psi_on_new_grid, Phi_on_new_grid] = isdftest.adaptive.adaptive_weight('get_wf_xga', ind_new);
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % CCH = isdftest.prod(Psi_on_grid(1:Nisdf_new, :), Psi_on_grid(1:Nisdf_new, :), ...
  ...
  idnew = isdftest.isdftest_add(isdf_data.desc);
  % Build a new ISDF object at idnew, keep original id unchanged.
  isdf_data_new = isdftest.get(idnew);
  ...
  for ikbz = 1:nbz
    ...
    if ikrot ~= 1
      wf_c_1 = isdftest.coeff.isdf_apply_symm_on_coarse(id, wf_c_1, ikrot);
    end
  ...
  isdftest.save2mod(isdf_data_new, idnew);
  % idnew = isdftest.isdftest_add(isdf_data.desc);
  isdftest.rsymm.bundle_refresh(id, Nextra, isdf_new_indices, idnew);
  % isdftest.rsymm.gen_bundle(idnew);
  ...
  % isdftest.changing(idnew);
  % isdftest.gen_tildeVq(idnew);
  % isdftest.validation.isdf_validation(idnew);

  isdftest.adaptive.adaptive_checkpoint_save(id, idnew);
  isdftest.numerical_cond_report('adaptive', id, idnew, isdf_data.desc, ...
