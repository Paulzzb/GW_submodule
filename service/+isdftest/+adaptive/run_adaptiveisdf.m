...
%
%   isdftest.adaptive.run_adaptiveisdf('nn', output_dir)
%   isdftest.adaptive.run_adaptiveisdf('vn', output_dir)
%
% Inputs:
%   type       鈥?'nn' or 'vn' (case-insensitive). Selects coarse ISDF slot description and nisdf rule.
%   output_dir 鈥?Directory where isdf_validate_HF writes the HF report via isdftest.report.gen_report
...
%   test_relay_stage.mat (e.g. test_Si), then call isdftest.adaptive.run_adaptiveisdf(...).
%
...
% validation uses that directory as pwd for report generation.
  ...
  if strcmp(type, 'vn')
    desc_token = 'vn';
    id_slot = isdftest.isdftest_add('vn');
    isdftest.set_nrange(id_slot, config.SYSTEM);
    slot_data = isdftest.get(id_slot);
    ...
    isdftest.save2mod(slot_data, id_slot);
  else
    desc_token = 'nn';
    id_slot = isdftest.isdftest_add('nn');
    isdftest.set_nrange(id_slot, config.SYSTEM);
    slot_data = isdftest.get(id_slot);
  ...
  isdftest.coeff.gen_coeff(cfg, id_slot);
  isdftest.coeff.print_coarse_grid_report(id_slot);
  ...
  isdftest.adaptive.isdf_schur_update('clear');
  isdftest.adaptive.adaptive_weight('clear');

  cleanup_cd = onCleanup(@() cd(profile_dir));
  cd(output_dir);
  isdftest.adaptive.adaptiveisdf(id_slot, cfg);
