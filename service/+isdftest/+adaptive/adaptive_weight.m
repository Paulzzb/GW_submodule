...
%   Mirror of isdftest.adaptive.isdf_schur_update: init / update / get / clear. Implementation TBD.
%
...
%   adaptive_weight('clear')
  ...
  switch lower(mode)
    ...
    case 'init_update'
      ...
      isdftest.adaptive.MrC1H('ensure', Nw, Nisdfmax);
      ...
      for ib = 1:n_batches
        ...
        cols_have = isdftest.adaptive.MrC1H('cached_cols', batch_idx_col);
        ...
        if any(need_mask)
          ...
          for is = 1:length(starts)
            ...
            cstart = cstart0 + 1;
            MrC1H_new = isdftest.prod(Psi(rows_now, :), Psi_on_grid(cstart:Nisdf, :), ...
                                  Phi(rows_now, :), Phi_on_grid(cstart:Nisdf, :));
            isdftest.adaptive.MrC1H('set_range', rows_now, cstart, MrC1H_new);
            isdftest.adaptive.MrC1H('set_cached_cols', rows_now, Nisdf);
            cols_have(row_mask) = Nisdf;
          end
        end
        MrC1H = isdftest.adaptive.MrC1H('get', batch_idx, 1, Nisdf);
    ...
    case 'update'
      ...
      for ib = 1:n_batches
        ...
        if Nold == 0
          MrC1H = isdftest.prod(Psi(batch_idx, :), Psi_on_grid(1:Nisdf, :), ...
                            Phi(batch_idx, :), Phi_on_grid(1:Nisdf, :));
          isdftest.adaptive.MrC1H('set_range', batch_idx, 1, MrC1H);
          isdftest.adaptive.MrC1H('set_cached_cols', batch_idx_col, Nisdf);
          ...
          if any(need_mask)
            ...
            for is = 1:length(starts)
              ...
              rows_now = batch_idx(row_mask);
              cstart = cstart0 + 1;
              MrC1H_old = isdftest.prod(Psi(rows_now, :), Psi_on_grid(cstart:Nold, :), ...
                                    Phi(rows_now, :), Phi_on_grid(cstart:Nold, :));
              isdftest.adaptive.MrC1H('set_range', rows_now, cstart, MrC1H_old);
              isdftest.adaptive.MrC1H('set_cached_cols', rows_now, Nold);
              cols_have(row_mask) = Nold;
            end
          end

          MrC2H = isdftest.prod(Psi(batch_idx, :), Psi_on_grid(Nold+1:Nisdf, :), ...
                            Phi(batch_idx, :), Phi_on_grid(Nold+1:Nisdf, :));
          isdftest.adaptive.MrC1H('set_range', batch_idx, Nold + 1, MrC2H);
          isdftest.adaptive.MrC1H('set_cached_cols', batch_idx_col, Nisdf);
          % invL2MrC2H = L_CCH(Nold+1:Nisdf, Nold+1:Nisdf) \ MrC2H;

          MrC1H = isdftest.adaptive.MrC1H('get', batch_idx, 1, Nold);
    ...
    case 'init'
      ...
      if isempty(isdf_data.nrange1) || isempty(isdf_data.nrange2)
        error('adaptive_weight:init:nrange', ...
          ['Missing cached nrange in ISDF id=%d. ' ...
           'Build it first via isdftest.set_nrange(id, config.SYSTEM).'], ...
          int32(id));
      end
      ...
      isdftest.adaptive.MrC1H('clear');
      % Calculate diag(MMH), fill them into w
      for ic = 1:Nw
        Psi_r = Psi(ic, :);
        Phi_r = Phi(ic, :);
        w(ic) = real(isdftest.prod(Psi_r, Psi_r, Phi_r, Phi_r));
      end
