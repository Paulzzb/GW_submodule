...
%   ok = isdf_schur_update('update', ...)   % false if Schur complement <= 0 (Nadd==1)
  ...
  switch lower(mode)
      ...
      if Nadd == 1
        ...
        if oldNisdf > 0
          if isempty(has_rank1_prod_mex)
            has_rank1_prod_mex = ~isempty(which('isdftest.adaptive.isdf_schur_rank1_prod_mex'));
          end
          if has_rank1_prod_mex
            try
              [L21_row, L22_mex, invL_new_old] = ...
                isdftest.adaptive.isdf_schur_rank1_prod_mex(invL_CCH, oldNisdf, ...
          ...
          if ~isempty(selected_global_idx)
            ...
            isdftest.adaptive.MrC1H('ensure', double(wf_data.nc), Nisdfmax);
            cols_have = isdftest.adaptive.MrC1H('cached_cols', selected_global_idx);
            if cols_have < oldNisdf
              cstart = cols_have + 1;
              C2C1H_new = isdftest.prod(Psi_on_new_grid, Psi_on_grid(cstart:oldNisdf, :), ...
                                    Phi_on_new_grid, Phi_on_grid(cstart:oldNisdf, :));
              isdftest.adaptive.MrC1H('set_range', selected_global_idx, cstart, C2C1H_new);
              isdftest.adaptive.MrC1H('set_cached_cols', selected_global_idx, oldNisdf);
            end
            C2C1H = isdftest.adaptive.MrC1H('get', selected_global_idx, 1, oldNisdf);
          else
            C2C1H = isdftest.prod(Psi_on_new_grid, Psi_on_grid(oldIdx, :), Phi_on_new_grid, Phi_on_grid(oldIdx, :));
          end
          C2C2H = isdftest.prod(Psi_on_new_grid, Psi_on_new_grid, Phi_on_new_grid, Phi_on_new_grid);

          if isempty(has_rank1_mex)
            has_rank1_mex = ~isempty(which('isdftest.adaptive.isdf_schur_rank1_mex'));
          end
          if has_rank1_mex
            try
              [L21_row, L22_mex, invL_new_old] = ...
                isdftest.adaptive.isdf_schur_rank1_mex(invL_CCH, oldNisdf, C2C1H, C2C2H);
              if isdf_schur_rank1_ok(L22_mex) && ...
                 isdf_schur_cond_guard_ok(real(L22_mex)^2, cch_lambda_max_proxy, max_cond_number, use_cond_guard, selected_global_idx)
                L_CCH(newIdx, oldIdx) = L21_row;
                L_CCH(newIdx, newIdx) = L22_mex;
    ...
    case 'init'
      ...
      CCH(1:Nisdf, 1:Nisdf) = isdftest.prod(Psi_on_grid(1:Nisdf, :), Psi_on_grid(1:Nisdf, :), ...
