...
%
%   [loaded, idnew] = isdftest.adaptive.adaptive_checkpoint_try_load(coarse_id)
%
...
% are used only when isdf_desc in the file matches the current coarse slot.
  ...
  fpath = isdftest.adaptive.adaptive_checkpoint_path(coarse_id, expected_desc);
  if exist(fpath, 'file') ~= 2
    legacy = isdftest.adaptive.adaptive_checkpoint_legacy_path(coarse_id);
  ...
  if ~isfield(S, 'isdf_data')
    error('isdf:adaptive:checkpoint:BadFile', ...
      'Checkpoint %s must contain variable isdf_data (isdftest.base.isdftest_m).', fpath);
  end
  if ~isa(S.isdf_data, 'isdftest.base.isdftest_m')
    error('isdf:adaptive:checkpoint:Type', ...
      'Checkpoint isdf_data must be isdftest.base.isdftest_m (got %s).', class(S.isdf_data));
  end
