...
% isdftest.report.gen_report  Write HF validation text report produced by isdf_validate_HF to a file.
%
%   fpath = isdftest.report.gen_report(report)
%   fpath = isdftest.report.gen_report(report, outDir)   % optional directory (default: pwd)
%
% Required fields on report:
%   isdf_id          鈥?ISDF pool id (integer)
%   band_summary     鈥?table with columns Item, Result
%   preview          鈥?table (may be empty)
%   stats            鈥?table of ob-layer statistics
%   n_samples        鈥?total ob sample count N
%   n_samples_rel    鈥?count used for relative-error streaming stats
%   preview_rows     鈥?preview row budget (min(N, preview_rows))
%   Esum2, EsumISDF2, DiffEsum2 鈥?accumulated sum of squares (same as validate_HF returns)
%
