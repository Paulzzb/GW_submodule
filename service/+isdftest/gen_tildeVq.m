...
% per q for isdftest.get_rho_xalpha (c_t = C^{-1/2} c).
...
%   trunc = inv_param, ratio = inv_ratio.
  ...
  if isempty(isdf_data.nrange1) || isempty(isdf_data.nrange2)
    error('isdftest:gen_tildeVq:nrange', ...
  ...
  isdftest.numerical_cond_report('gen_tildeVq_begin', id, isdf_data.desc, s_cut);
  ...
  for iqibz = 1:nibz
    ...
    for ikbz = 1:nbz
      ...
      for ind_ib = 1:nb1
        ...
        wf_c_1(:, ind_ib) = isdftest.get_u_xalpha(id, isc1);
      end
      for ind_ib = 1:nb2
        ib = nrange2(ind_ib);
        isc2 = int32([ib, ikpibz, ikprot, 1]);
        wf_2(:, ind_ib) = wave_functions.WF_apply_symm(isc2);
        wf_c_2(:, ind_ib) = isdftest.get_u_xalpha(id, isc2);
      end
      wf_1 = conj(wf_1);
      wf_c_1 = conj(wf_c_1);
    ...
    l_keep = isdftest.prod_C_inv_t('set', CCHq, s_cut, ratio);
    isdf_data.CCHq(:, :, iqibz) = CCHq;
    [V_trunc_iq, Lambda_trunc_iq] = isdftest.prod_C_inv_t('get_factors');
    fac = struct();
    ...
    isdftest.numerical_cond_report('gen_tildeVq_iq', id, iqibz, CCHq, MCHq, l_keep, s_cut);

    helperqR = isdftest.prod_C_inv_t('prod', 'r', MCHq);
  ...
  fprintf('[ISDTEST] gen_tildeVq done for id=%d (SVD C^{-1/2} path, s_cut=%g).\n', int32(id), s_cut);
