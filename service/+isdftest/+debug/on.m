...
%       Uses flags snapshotted by isdftest.debug.init_from_config(config) (e.g. from
%       service_driver). No config argument.
%
%   tf = isdftest.debug.on(config, tag)   % optional overload
...
%   debug_level is only used by react/level, not by on.
  ...
  if nargin ~= 1
    error('isdf:debug:onArgs', 'Use isdftest.debug.on(tag) or isdftest.debug.on(config, tag).');
  end
