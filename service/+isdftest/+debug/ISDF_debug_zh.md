...
- **`isdftest.debug.init_from_config(config)`** 在 **`service_driver`** 启动时调用；在 **`isdftest.adaptive.run_adaptiveisdf`** 中于 relay `restore` 之后也会调用（不经过 `service_driver` 的路径）。
- 初始化之后，各调用点使用 **`isdftest.debug.on(tag)`** 与 **`isdftest.debug.react(...)`**，**不再传入 `config`**。
- **`isdftest.debug.clear()`** 清空缓存；已在 **`service_reset_persistent`** 中注册，与其它 ISDF 持久态一并清理。
...
放在**昂贵计算之外**（`if isdftest.debug.on(tag) ... end`）。**仅**使用会话缓存。若从未调用 `init_from_config`，恒为 `false`。
...
- **`isdftest.debug.on(config, tag)`** — 直接读 `config.ISDF`；**不会**写回会话缓存。
- **`isdftest.debug.level(config)`** / **`isdftest.debug.level()`** — 从传入的 config 或缓存取等级。
- **`isdftest.debug.react(config, violation, msg [, tagForId])`**
- **`isdftest.debug.check(tag, fh, msg [, id])`** 或 **`isdftest.debug.check(config, tag, fh, msg [, id])`**

### 内部接口

- **`isdftest.debug.cache`** — 持久存储；一般业务代码勿直接依赖。
...
在拿到与传给 `service_driver` **相同**的 `config` 结构体之后，自行调用一次 **`isdftest.debug.init_from_config(config)`**（例如 `load(..., 'config')` 之后）。
