...
%
%   isdftest.debug.react(violation, msg)
%   isdftest.debug.react(violation, msg, tagForId)
%       Uses session cache from init_from_config for level().
%
%   isdftest.debug.react(config, violation, msg [, tagForId])   % optional overload
%       Uses config.isdftest.debug_level only.
  ...
  if nargin >= 3 && isa(varargin{1}, 'struct') && isfield(varargin{1}, 'ISDF')
    ...
    lev = isdftest.debug.level(config);
  elseif nargin >= 2
