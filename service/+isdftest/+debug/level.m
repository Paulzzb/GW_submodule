...
%LEVEL  Normalized reaction level ('error' or 'warn') for isdftest.debug.react.
%
%   lev = isdftest.debug.level()  鈥?from session cache (init_from_config).
%   lev = isdftest.debug.level(config)  鈥?from config.ISDF only (no cache read).
