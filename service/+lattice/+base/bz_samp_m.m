...
classdef bz_samp_m
  properties
    ...
    %------------------------------------------------------------
    % b1b2b3(3, 3)   = double(zeros(3, 3))  % Reciprocal lattice matrix (rows = b1,b2,b3)

    % kpt(nibz, 3)   : RLU k-points  (alias for pt, Cartesian)
    kpt_RLU(:, 3)     = double(zeros(0, 3))
    kpt_Cart(:, 3)     = double(zeros(0, 3))
    ...
    kptbz_RLU(:, 3)   = double(zeros(0, 3))
    kptbz_Cart(:, 3)   = double(zeros(0, 3))

    bz2ibz(:, 1)        {mustBeInteger} = int32(zeros(0, 1))  % ik_bz -> ik_ibz
    bz2rot(:, 1)        {mustBeInteger} = int32(zeros(0, 1))  % ik_bz -> isym 
    ibz2bz(:, 1)        {mustBeInteger} = int32(zeros(0, 1))  % (ik_ibz, 1) -> ik_bz
  ...
  methods
    function obj = bz_samp_m(nibz, nsym)
      % bz_samp_m(nibz, nsym)
      ...
      obj.kpt_RLU   = double(zeros(nibz, 3));
      obj.kpt_Cart = double(zeros(nibz, 3));
      % obj.kptbz_RLU   = double(zeros(nbz, 3));
      % obj.kptbz_Cart = double(zeros(nbz, 3));
      obj.weights = double(zeros(nibz, 1));
