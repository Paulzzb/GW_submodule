...
%
% Minimal smoke test for Si32.save under nok-style settings.
%
% Usage (MATLAB):
%   cd GW/test_profile/test_nok_si32
...
function E = test_gw_cohsex_multi_k(profileflag)
  ...
  fprintf('\n[test_nok_si32] Running qp_cohsex(config) ...\n');
  t_qp = tic;
  E = qp_cohsex(config);
  wall_qp = toc(t_qp);
  if profileflag
  ...
  fprintf('\n[test_nok_si32] Wall clock (tic/toc):\n');
  fprintf('  input_driver + load(config): %.3f s\n', wall_input);
  fprintf('  qp_cohsex (Ex + COHSEX):      %.3f s\n', wall_qp);
  fprintf('  total:                        %.3f s\n', wall_input + wall_qp);
  if profileflag
    fprintf('[test_nok_si32] Profiler HTML:\n');
    fprintf('  %s\n', fullfile(cpath, 'profile_input'));
    fprintf('  %s\n', fullfile(cpath, 'profile_qp'));
  end

  fprintf('\n[test_nok_si32] size(E.Eqp)    = [%d %d]\n', size(E.Eqp, 1), size(E.Eqp, 2));
  fprintf('[test_nok_si32] size(E.Ex)     = [%d %d]\n', size(E.Ex, 1), size(E.Ex, 2));
  fprintf('[test_nok_si32] size(E.Esx_x)  = [%d %d]\n', size(E.Esx_x, 1), size(E.Esx_x, 2));
  fprintf('[test_nok_si32] size(E.Ecoh)   = [%d %d]\n', size(E.Ecoh, 1), size(E.Ecoh, 2));
  fprintf('[test_nok_si32] ||E.Eqp||_F    = %.6e\n', norm(E.Eqp, 'fro'));
  fprintf('[test_nok_si32] ||E.Esx_x||_F  = %.6e\n', norm(E.Esx_x, 'fro'));
  fprintf('[test_nok_si32] ||E.Ecoh||_F   = %.6e\n', norm(E.Ecoh, 'fro'));
end

function local_profsave_html(s, destDir)
  if isempty(s)
    warning('test_nok_si32:profsave', ...
