...
%
% Output columns are (fileB - fileA). Default fileA = qp_cohsex.dat,
% fileB = qp_ff_isdf.dat, outFile = qp_ff_minus_cohsex.dat in this folder.

if nargin < 1 || isempty(fileA)
  fileA = fullfile(fileparts(mfilename('fullpath')), 'qp_cohsex.dat');
end
if nargin < 2 || isempty(fileB)
  fileB = fullfile(fileparts(mfilename('fullpath')), 'qp_ff_isdf.dat');
end
if nargin < 3 || isempty(outFile)
  outFile = fullfile(fileparts(mfilename('fullpath')), 'qp_ff_minus_cohsex.dat');
end
...
dRe = ReB - ReA;
dIm = ImB - ImA;
% band index column: keep reference from B, zero diff expected
dRe(:, 1) = ReB(:, 1);
