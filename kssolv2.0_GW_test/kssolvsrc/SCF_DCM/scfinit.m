function [mol,H,X,Hprec,varargout] = scfinit(mol,varargin)
% SCFINIT generates initial values for the iterative method.
%    [mol,H,X,Hprec] = SCFINIT(mol) generates the initial Hamiltonian,
%    wave functions and preconditioner of the corresponding Hamiltonian for
%    molecule mol.
%
%    [mol,H,X,Hprec] = SCFINIT(mol,rho) generates the initial Hamiltonian,
%    wave functions and preconditioner of the corresponding Hamiltonian for
%    molecule mol together with density function rho.
%
%    [mol,H,X,Hprec] = SCFINIT(mol,X) generates the initial Hamiltonian,
%    wave functions and preconditioner of the corresponding Hamiltonian for
%    molecule mol together with wave function X.
%
%    [mol,H,X,Hprec] = SCFINIT(mol,rho,X) generates the initial
...
%    wave function X.
%
%    [mol,H,X,Hprec] = SCFINIT(mol,rho,X,ncol) generates X with ncol
%    columns.
%
%    [cry,BH,BX,BHprec] = SCFINIT(cry) generates the initial Bloch
%    Hamiltonian, Bloch wave functions and the corresponding preconditioner
%    for the given crystal cry.
%
%    [cry,BH,BX,BHprec] = SCFINIT(cry,rho) generates the initial Bloch
%    Hamiltonian, Bloch wave functions and the corresponding preconditioner
%    for the given crystal cry together with density function rho.
%
%    [cry,BH,BX,BHprec] = SCFINIT(cry,BX) generates the initial Bloch
%    Hamiltonian, Bloch wave functions and the corresponding preconditioner
%    for the given crystal cry together with Bloch wave function BX.
%
%    [cry,BH,BX,BHprec] = SCFINIT(cry,rho,BX) generates the initial Bloch
%    Hamiltonian, Bloch wave functions and the corresponding preconditioner
%    for the given crystal cry together with density function rho and Bloch
%    wave function BX.
%
%    [cry,BH,BX,BHprec] = SCFINIT(cry,rho,BX,ncols) generates BX with each
%    number in ncols being the number of columns for each k-points.
%
%    [...,nocc] = SCFINIT(...) returns the total number of occupied
