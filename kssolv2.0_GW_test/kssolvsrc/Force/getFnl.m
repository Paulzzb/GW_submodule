...
if isa(mol,'Crystal')
    ...
    function Fnlsingle = getFnlsingle(bec,sig,X)
        ...
        dbecX = bec'*bsxfun(@times,1i*gkx,X);
        dbecXbecX = real(becX.*conj(dbecX));
        Fx = -4*sum(bsxfun(@times,sig,dbecXbecX),2);
        
        dbecX = bec'*bsxfun(@times,1i*gky,X);
        dbecXbecX = real(becX.*conj(dbecX));
        Fy = -4*sum(bsxfun(@times,sig,dbecXbecX),2);
        
        dbecX = bec'*bsxfun(@times,1i*gkz,X);
        dbecXbecX = real(becX.*conj(dbecX));
        Fz = -4*sum(bsxfun(@times,sig,dbecXbecX),2);
