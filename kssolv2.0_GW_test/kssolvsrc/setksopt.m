...
%                    'davidson'           with the lowest (occupied) states
%                    'ppcg'               of a fixed KS Hamiltonian
...
%
%      relaxmethod   'fminunc'            Optimization method used for geometry
%                     'nlcg'               optimization or atomic structure 
%                     'bfgs'               relaxation
%                               
%      relax_tol     positive float       convergence tolerance for geometry
%                                         optimization

if (nargin == 0) && (nargout == 0)
    fprintf('                verbose: [ off | on ]\n');
    fprintf('              eigmethod: [ lobpcg | eigs | davidson | ppcg ]\n');
    ...
    fprintf('                  force: [ bool ]\n');
    fprintf('            relaxmethod: [ fminunc | NLCG | BFGS ]\n');
    fprintf('               relaxtol: [ positive float ]\n');
...
allfields = {'verbose';'eigmethod';'maxscfiter';'maxdcmiter';...
             ...
             'ishybrid';'useace';'Vexx';'maxphiiter';'phitol';...
             'dfrank';'ncbands';...
             'relaxmethod';'relaxtol';'factorOrbitals';'davsteps'};
...
options.degree = 10;
options.force = 1;
options.relaxmethod = 'fminunc';
options.relaxtol = 1e-4;
options.factorOrbitals = 1;
options.ishybrid = 0;
options.useace = 0;
options.Vexx = [];
options.maxphiiter = 5;
options.phitol = 1e-8;
options.dfrank = 0;
options.ncbands = 0;
options.ngbands = 0;
...
switch field
    ...
    case {'eigmethod'} % lobpcg,eigs,davidson, ppcg
        [validvalue, errmsg, errid] = eigType(field,value);
    case {'scftol','dcmtol','cgtol','eigstol','betamix','factorOrbitals','phitol'} % real scalar
        [validvalue, errmsg, errid] = nonNegReal(field,value);
    case {'maxscfiter','maxdcmiter','maxinerscf','maxcgiter','maxeigsiter','brank','mixdim','maxphiiter','davsteps'} % integer 
    ...
    case {'degree', 'dfrank', 'ncbands'}  %integer
        [validvalue, errmsg, errid] = nonNegInteger(field,value);
    case {'force','useace','ishybrid'}  %boolean integer
        [validvalue, errmsg, errid] = nonNegInteger(field,value);
    case {'Vexx'}
        validvalue = 1;
    case {'relaxmethod'}  %string
        [validvalue, errmsg, errid] = relaxMethod(field,value);
    case {'relaxtol'}  %float
        [validvalue, errmsg, errid] = nonNegReal(field,value);
    otherwise
        validfield = false;  
...
if ~valid
    ...
    if (any(strcmp(lower(value),{'lobpcg';'eigs';'chebyfilt';'davidson'})))
       errid = '';
       errmsg = '';
    else
       valid = 0;
       errid = 'MATLAB:funfun:setksopt:eigType';
       errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following: lobpcg, eigs, chebyfilt, davidson\n',field);
    end
end
%-------------------------------------------------------------------------------
...
%-------------------------------------------------------------------------
function [valid, errmsg, errid] = relaxMethod(field,value)
% check for mixing type
valid =  ischar(value);
if ~valid
    errid = 'MATLAB:funfun:setksopt:eigType';
    errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following strings: fminunc, NLCG, BFGS',field);
else
    if (any(strcmp(lower(value),{'fminunc';'nlcg';'bfgs'})))
       errid = '';
       errmsg = '';
    else
       valid = 0;
       errid = 'MATLAB:funfun:setksopt:relaxMethod';
       errmsg = sprintf('Invalid value for OPTIONS parameter %s: must be one of the following: fminunc, NLCG, BFGS\n',field);
    end
end;
