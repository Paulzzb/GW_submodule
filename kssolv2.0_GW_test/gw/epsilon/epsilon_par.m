function eps = epsilon_par(sys, options, syms, eps)
...
for iq = 1 : sys.nkpts
    ...
    for ik = 1 : nrq
        ...
        fprintf('Calculating M matrix element ...\n');
        for ispin = 1:nspin
            ...
            % 创建临时存储
            temp_gme = cell(no_v, nbands-no_c+1);
            parfor iv = 1:no_v
                for ic = no_c:nbands
                    temp_gme{iv, ic} = getm_epsilon(iv, ic, wfnkq, wfnk, iq, ispin, pol, sys, options);
                end
            end
            ...
            for iv = 1:no_v
                for ic = no_c:nbands
                    gme{iv,ic,indrk(ik),iq,ispin} = temp_gme{iv, ic};
                    ...
                    if (nstar > 1)
                        for it = 2:nstar
                            ...
                            gme{iv,ic,ik_degenerate,iq,ispin} = temp_gme{iv, ic}(indt{it, ik});
                        end
                    end
                end
            end
...
for iq = 1:sys.nkpts
    ...
    for ispin = 1:nspin
        gme_q = gme(:, :, :, iq, ispin);
        chi0{iq, ispin} = get_chi0(gme_q) * fact;
    end
    if (nspin == 2)
