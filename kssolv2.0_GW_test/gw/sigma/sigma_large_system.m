function sig = sigma_large_system(eps, sig, sys, options, syms)
    ...
    for in = 1 : ndiag_max
        ...
        for ik = 1 : sig.nkn
            ...
            for iq = 1 : nrk
                ...
                coulg = getvcoul(fbz.nmtx(indrk(iq)), fbz.isrtx(:, indrk(iq)), ekin(:, indrk(iq)), coul_cutoff, 1);
            ...
            % Store final results
            if sig.exact_static_ch
                sig.cor(in,ik,ispin) = real(asx_sum + 0.5*achx_sum) * ryd;
                sig.sig(in,ik,ispin) = real(asx_sum + ax_sum + 0.5*achx_sum) * ryd;
            else
                sig.cor(in,ik,ispin) = real(asx_sum + 0.5*ach_sum) * ryd;
                sig.sig(in,ik,ispin) = real(asx_sum + ax_sum + 0.5*ach_sum) * ryd;
            end
        end
    end
end
