        ...
        for ik = 1 : sig.nkn
            ...
            for iq = 1 : nrk
                ...
                for nn = 1 : nbands
                    ...
                    %% Calculate SX and X
                    if occ_kq(nn) > 0
                    %asx_loc = asx_loc - occ_kq(nn) * aqs_cutoff.* aqs_cutoff'.* eps_inv * coulg_cutoff;
                    asx_loc = asx_loc - occ_kq(nn) * aqs_cutoff.* aqs_cutoff'.* (eps_inv - I) * coulg_cutoff; % this is actually asx - x;
                    ax_loc = ax_loc - occ_kq(nn) * abs(aqs_nocut).^2 .* coulg_nocut;
                    end
                    %% Calculate CH without exact ch correlation
                    ach_loc = ach_loc + 0.5 * aqs_cutoff.* aqs_cutoff'.* (eps_inv - I) * coulg_cutoff;
                end
                ...
                if sig.exact_static_ch
                    ...
                    %aqsch_tmp(abs(eps_inv_I) < TOL_Small) = 0; no need
                    
                    achx_loc = 0.5 * aqsch_tmp .* eps_inv_I * coulg_cutoff;
            ...
            asx(in,ik,ispin) = sum(asxtemp,"all");
            ax(in,ik,ispin) = sum(axtemp,"all");
            ach(in,ik,ispin) = sum(achtemp,"all");
            if sig.exact_static_ch
                achx(in,ik,ispin) = sum(achxtemp,"all");
            end
        end
    end
end
