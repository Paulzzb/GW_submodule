function [wfnkq] = genwf(rkq, gr, gvec, structure, syms, sys, options, cutoff)
...
if (nmtx ~= structure.nmtx(ikrkq))
    error('G matrix dimension of rkmatch of is not correct')
end
...
ind = gmap(gvec, syms, structure.nmtx(:,ikrkq), itqq, kgqq, isrtc_kq ,isorti, sys); % The index connect G of rkq and the one in ikrkq WFN
%% assignment
wfnkq.ikq = ikrkq;
