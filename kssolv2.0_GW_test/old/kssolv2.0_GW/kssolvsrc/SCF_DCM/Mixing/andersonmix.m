function [vnew,df,dv] = andersonmix(mol, vin, vout, beta, df, dv, iter, mixdim)
%
% usage: [vnew,df,dv] = andersonmix(mol, vin, vout, beta, df, dv, iter, mixdim)
%
...
if (iter > 1)
   ...
   %B = DPCoulomb(mol, df(:,1:iterused), df(:,1:iterused));
   B = (B+B')/2;
   % 
   work = df(:,1:iterused)'*reshape(vout,n123,1);
   %work = DPCoulomb(mol, df(:,1:iterused), reshape(vout,n123,1));
   %rcondB  =rcond(B)
   %pause
   %gammas = B\(work);
   gammas = real(pinv(B)*(work));
   %
