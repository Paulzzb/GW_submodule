...
[cry,BH,BX,BHprec,noccs] = scfinit(cry,rho,BX);
...
for iterscf = 1:maxscfiter
    ...
    % adative convergence tolerance for eigensolver
    cgtol0 = options.cgtol;
    options.cgtol = min(vtoterr/cry.nel,cgtol0);
    options.cgtol = max(options.cgtol, options.scftol/10);
    fprintf('eigtol = %11.3e\n', options.cgtol);
    idx = 0;
    for ik = 1:nkpts
        idx = idx(end) + (1:nBXcols(ik));
        [X, ev(idx)] = KSeigs(cry, BH{ik}, BX{ik}, BHprec{ik}, options);
        BX{ik} = X;
    end
    info.Eigvals = ev;
