...
grid  = Ggrid(mol);
if isa(mol,'Crystal') && mol.modifyidxnz
    idxnz = mol.qeidxnz;
    ng   = length(idxnz);
    gkx  =  grid.kkxyz(idxnz,1)+kpts(1);
    gky  =  grid.kkxyz(idxnz,2)+kpts(2);
    gkz  =  grid.kkxyz(idxnz,3)+kpts(3);
else
    ng    = grid.ng;
    gkx   = grid.gkx+kpts(1);
    gky   = grid.gky+kpts(2);
...
for itype = 1:ntypes
    ...
    px = repmat(gk/dq-floor(gk/dq),1,nbeta);
    ux = 1-px;
    vx = 2-px;
    wx = 3-px;
    it0 = floor(gk/dq)+1;
...
for it = 1:na   
    ...
    if size(vnlcell{it},2) > nbetamax
        nbetamax = size(vnlcell{it},2);
    end
end
...
if ~mol.noncolin
    ...
    % as for noncolinear spin case, vnlD may not be symmetry so 
    % VV' = I maybe false and the simplication above doesn't work
    % every type of atom corresponds to four D matrixs here
    vnlmat  = zeros(length(gk),len);
    vnlsign = cell(4,1);
    for i = 1:4
        vnlsign{i} = zeros(len);
    ...
    for it = 1:na
        idx = offset + (1:size(vnlcell{it},2));
        for is = 1:4
            vnlsign{is}(idx,idx) = vnlD{it}(:,:,is);
        end
        vnlmat(:,idx) = vnlcell{it};
        offset =  offset + size(vnlcell{it},2);
    end
end   
end
