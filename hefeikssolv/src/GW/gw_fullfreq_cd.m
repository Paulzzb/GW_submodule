function [Eqp0,Eqp1]=gw_fullfreq_cd(ksinfo, options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% real_freq = ksinfo.real_freq;
% imag_freq = ksinfo.imag_freq;
...
if 1
  ...
  for ifreq = 1:nFreq
  	...
  	ksinfo.coulG(:, 4) .* epsilon(:, :, ifreq) / vol;
  end
...
if 0
  ...
  if ~isempty(ksinfo.aqs)
    for ind_nv = nv-nv_oper+1:nv
      ...
      Mgvc(:, (1:nc_oper) + (ind_nv_true-1) * (nc_oper)) ...
       = ksinfo.aqs{ind_nv}(:, nv+1:nv+nc_oper);
    end
  else
    aqs = cell(nv + nc, 1);
  ...
  %  gvec = ksinfo.gvec;
    for ind_aqs = 1:nv+nc
      aqstmp = mtxel_sigma(ind_aqs, ksinfo, options.Groundstate, ...
          1:nv+nc);
      aqs{ind_aqs} = aqstmp;
    end
    ksinfo.aqs = aqs;
  ...
  for ifreq = 1 : nFreq 
    ...
    ( 1.0 ./ (Eden - (dFreqGrid(ifreq) + dFreqBrd(ifreq))) ... 
    + 1.0 ./ (Eden + (dFreqGrid(ifreq) + dFreqBrd(ifreq))) );
    epsilon(:,:,ifreq) = eye(ng) - 4 * ksinfo.coulG(:,4) .* Mgvc * (edenDRtmp .* Mgvc') / vol;
  end
...
if testflag3
  coeff_matrix = zeros(n_ener, n_oper, nfreq_imag);
  sint_out = zeros(n_ener, n_oper, nfreq_imag);
  aqslist = [];
  coeff_real = zeros(n_ener, n_oper, nfreq_real);
  sres_elements = zeros(n_ener, n_oper, nfreq_real);
end
for ibandouter = nv-nv_ener+1 : nv+nc_ener
  ...
  % aqsntemp = ...
  % ksinfo.aqs{ibandouter_aqsntemp};
...
%   pause(1);
    ...
    for iw = 1:options.GWCal.nfreqeval
      ...
      if ( (wx >= 0.0)  == flagocc)
        continue;
        if (iw == (options.GWCal.nfreqeval+1)/2)
          sres_elements(ibandouter_aqsntemp, ibandinner_aqsntemp, :) = 0;
          coeff_real(ibandouter_aqsntemp, ibandinner_aqsntemp, :) = 0;
        end
      end
      occ_sign = wx / abs(wx);
      wx = abs(wx);
...
%      fprintf('fact1 = %f, fact2 = %f\n', fact1, fact2);
      if testflag3 
        if (iw == (options.GWCal.nfreqeval+1)/2)
          coeff_real(ibandouter_aqsntemp, ibandinner_aqsntemp, ifreq) = fact1;
          coeff_real(ibandouter_aqsntemp, ibandinner_aqsntemp, ifreq+1) = fact2;
          for ifreq_ = 1:nfreq_real
            sres_elements(ibandouter_aqsntemp, ibandinner_aqsntemp, ifreq_) = ...
            aqsntemp(:, ibandinner_aqsntemp)' * I_eps_array(:, :, ifreq_) ...
            * Dcoul * aqsntemp(:, ibandinner_aqsntemp) / ksinfo.vol;
          end
        end
      end
      ...
      if nfreq_real > 1
        ...
        * Dcoul * aqsntemp(:, ibandinner_aqsntemp) / ksinfo.vol;
        ...
        sres_omega = sres_omega + aqsntemp(:, ibandinner_aqsntemp)' ...
                   * I_eps_array(:, :, 1) * Dcoul / ksinfo.vol * aqsntemp(:, ibandinner_aqsntemp);
      end 
...
%      end %for igcol = 1:ng       
    ...
    if testflag1
      aqslist = [aqslist, aqsntemp(:, ibandinner_aqsntemp)];
    end
    for iw = 1:nfreq_imag
      sW_imag_freqs(iw) = sW_imag_freqs(iw) + aqsntemp(:, ibandinner_aqsntemp)' ...
                   * I_eps_array(:, :, iw+nfreq_real) * Dcoul / ksinfo.vol...
                   * aqsntemp(:, ibandinner_aqsntemp);
    end
    ...
    switch options.GWCal.cd_int_method
      case 0
        for ifreq = 1 : options.GWCal.nfreq_imag 
          ...
          if testflag3
            ind = (length(wxi)+1)/2;         
            coeff_matrix(ibandouter_aqsntemp, ibandinner_aqsntemp, ifreq) = ...
            (atan(freqEnd ./ wxi(ind)) - atan(freqStart ./ wxi(ind)) );
            if (ibandouter == 2 && ibandinner == 3)
              1;
            end
            sint_out(ibandouter_aqsntemp, ibandinner_aqsntemp, ifreq) = ...
            sW_imag_freqs(ifreq);
          end
...
% save('intcoeff.mat', coeff_matrix)
Eres = Esx_x(:, (options.GWCal.nfreqeval + 1)/2)
Eint = Ech(:, (options.GWCal.nfreqeval + 1)/2)
Ex = real(Ex);

if testflag3
  save('I_eps_array.mat', 'I_eps_array');
  save('sint_n_i_omega.mat', 'sint_out');
  save('aqslist.mat', 'aqslist');
  save('coeff_real.mat', 'coeff_real');
  save('sres_elements.mat', 'sres_elements')
end
GWenergy = struct();
