function [d,Vs] = bse(ksinfo,options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
bdot  = ksinfo.bdot;
G     = ksinfo.G(:,1:3);
plasma_omega = ksinfo.plasma_omega;
nvbands = ksinfo.nvbands;
ncbands = ksinfo.ncbands;
nbands = nvbands + ncbands;
NBANDS = ksinfo.NBANDS;
ry2ev = 13.60569253;

%parameters for head/wing/body/exchange of bse Hamiltonian
fac_d = - 8*pi/vol;
w_eff = ksinfo.coulG0/(8*pi);
...
I = eye(ng);
%chi0(logical(eye(ng)))=diag(chi0).*ksinfo.coulG(:,4);
%chi0_coul=chi0;
