function gw_fullfreq_cd(ksinfo, options)
...
if (options.ISDFCauchy.isISDF)
  Eres = gw_fullfreq_cd_res_ISDF(ksinfo, options);
  Eint = gw_fullfreq_cd_int_ISDF(ksinfo, options);
else
  Eres = gw_fullfreq_cd_res(ksinfo, options);
  Eint = gw_fullfreq_cd_int(ksinfo, options);
end

Ex = gw_x(ksinfo, options);
Vxc = ksinfo.Vxc * options.Constant.ry2ev;
ev  = ksinfo.ev(1:length(Ex))  * options.Constant.ry2ev;
