function gw_cohsex(ksinfo, options)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (options.ISDFCauchy.isISDF)
  ...
  for i = 1:vcrank_mu
    ...
    fftbox1 = do_FFT(fftbox1, gvec.fftgrid, 1) * ksinfo.vol;
  ...
  [ssrzeta_mu, ssind_mu] = isdf(conj(psir(:,nv-nv_oper+1:nv+nc_oper)), ...
                                (psir(:,nv-nv_ener+1:nv+nc_ener)), optionsISDF);
  % [ssrzeta_mu, ssind_mu] = isdf_gw(conj(psir(:,nv-nv_oper+1:nv+nc_oper)), ...
  %                               (psir(:,nv-nv_ener+1:nv+nc_ener)), ssrank_mu);
  ...
  if options.ISDFCauchy.isCauchy
    Phi = psir(vcind_mu, nv-nv_oper+1:nv); 
  ...
  %  scal = 4.0 / ksinfo.vol; % for kpoints & spin, change it according to
    ...
    for ind_nv = nv-nv_oper+1:nv
      Mgvc = conj(psir(vcind_mu, ind_nv)) .* psir(vcind_mu, nv+1:nv+nc_oper);
      Mgvc = conj(Mgvc);
      eden = 1 ./ (ev(ind_nv) - ev(nv+1:nv+nc_oper));
      COmegaCresult = COmegaCresult + scal * Mgvc * diag(eden) * Mgvc';
    end
  end
  COmegaCresult = COmegaCresult / ksinfo.vol;
  % Calculate ( (C*Omega*C)^(-1) ...
  %                               - vcgzeta_mu' * Dcoul * vcgzeta_mu)^(-1) 
  invepsg_main = inv( inv(COmegaCresult) - vcgzeta_mu' * Dcoul * vcgzeta_mu); 
  ...
  Phivs = psir(vsind_mu, nv-nv_oper+1:nv); 
  Phiss = psir(ssind_mu, nv-nv_oper+1:nv+nc_oper); 
  ...
  clear W1_mu vcDcoulvs vcDcoulss;
  ...
  timeForSelfE = tic;
  Psivs = conj(psir(vsind_mu, nv-nv_ener+1:nv+nc_ener)); 
  Psiss = conj(psir(ssind_mu, nv-nv_ener+1:nv+nc_ener)); 
  
  Esx_x = Psivs' * Sigma_sex_x * Psivs / ksinfo.vol;
  Ech   = Psiss' * Sigma_coh   * Psiss / ksinfo.vol;
  Ex    = Psivs' * Sigma_x     * Psivs / ksinfo.vol;
...
%%%%%%%%%%%%%
  ...
  %   Mgvn = (Mgvn);
  %   Ex_2 = Ex_2 + Mgvn' * vsDcoulvs * Mgvn / ksinfo.vol;
  % end
  % clear Mgcn;
...
%   timeForExactCH = toc(startExactCH)
  ...
  %   achx = achx + aqsch(iout_list(gindex_list), :).' * W1(gindex_list, igcol) / 2 / ksinfo.vol;
  % end % for igcol = 1:ng
  %  timeForExactCH = toc(startExactCH);
...
%   ks2bgw = index_ks2bgw(ksinfo.coulG(:, 1:3), gvec_bgw(1:81, :));
%   [~, bgw2ks] = sort(ks2bgw);
 
  startforW = tic;
  if ~isfield(ksinfo, 'aqs')
    aqsFlag = false;
  elseif isempty(ksinfo.aqs);
    aqsFlag = false;
  else
    aqsFlag = true;
  end
...
%  scal = 4.0 / ksinfo.vol; % for kpoints & spin, change it according to
  ...
  for ind_nv = nv-nv_oper+1:nv
    if aqsFlag
      ...
      % Mgvc = nr .^ 3 * mtxel_sigma(ind_nv, ksinfo, options.Groundstate, (nv+1:nv+nc_oper));
...
%   W1_     = (inveps + eye(ng) * Dcoul;
  ...
  for ioper = nv-nv_oper+1:nv
    ...
    Mgvn = conj(Mgvn);
    Esx_x = Esx_x + Mgvn' * W1 * Mgvn / ksinfo.vol;
    Ex = Ex + Mgvn' * Dcoul * Mgvn / ksinfo.vol;
    Ech = Ech + Mgvn' * W1 * Mgvn / ksinfo.vol;
  end
  clear Mgvn;
  for ioper = nv+1:nv+nc_oper
    if aqsFlag
      Mgcn = ksinfo.aqs{ioper}(:, nv-nv_ener+1:nv+nc_ener);
    else
      Mgcn = mtxel_sigma(ioper, ksinfo, options.Groundstate, (nv-nv_ener+1:nv+nc_ener));
    end
    Mgcn = conj(Mgcn);
    Ech = Ech + Mgcn' * W1 * Mgcn / ksinfo.vol;
  end
  clear Mgcn;
...
%
%  for igcol = 1 : ng
%    fftbox1 = put_into_fftbox(W1(:, igcol) * ksinfo.vol, gvec.idxnz, gvec.fftgrid);
%    fftbox1 = gvec.nfftgridpts / ksinfo.vol * do_FFT(fftbox1, gvec.fftgrid, 1);
...
%             [ksinfo.mol.n1, ksinfo.mol.n2, ksinfo.mol.n3] * 2 * pi;
...
%    achx = achx + (sum(W_rg_tmp .* aqsch, 1))' * ksinfo.vol / gvec.nfftgridpts;
%  end
...
%  for iener = 1:1
%    if aqsFlag
%      aqsch = ksinfo.aqs{iener}(:, iener);
%    else
%      aqsch = mtxel_sigma(iener, ksinfo, options.Groundstate, iener);
%    end
...
%       end % for igrow
%       achx(iener) = achx(iener) + achxtemp_gp * vcoul(igcol) * 0.5 / ksinfo.vol;
%     end % for igcol
%   end % for iener
% Exact CH   
  ...
  % for iener = nv-nv_ener+1 : nv+nc_ener
  %   if aqsFlag
  %     aqsch(:, iener-nv+nv_ener) = ksinfo.aqs{iener}(:, iener);
  %   else
  %     aqsch(:, iener-nv+nv_ener) = mtxel_sigma(iener, ksinfo, options.Groundstate, iener);
  %   end
  % end
