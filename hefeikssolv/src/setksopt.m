...
%     continue       bool                 false: from scratch, true: provide X0 and rho0
%    exxmethod       'normal'             Method to calculate hybrid functional
%					 'qrcp'       
%					 'kmeans'
%      aceconv       bool                 Whether perform Fourier convolution in ACE.
...
allfields = {'verbose';'eigmethod';'maxscfiter';'maxdcmiter';...
             ...
             'factorOrbitals';'ishybrid';'Vexx';'maxphiiter';'phitol';...
             'dfrank';'ncbands';'continue';'exxmethod';'isdf_threshold';...
			 'aceconv';'x_gamma_extrapolation';'GACE'};
...
% set default values
options.verbose = 0;
options.eigmethod = 'davidson_qe';
options.maxscfiter = 30;
...
options.what2mix = 'rho';
options.mixtype = 'broyden_modify';
...
options.ishybrid = 0;
options.Vexx = [];
options.maxphiiter = 15;
options.phitol = 1e-8;
options.dfrank = 0;
options.continue = ~(isempty(options.X0)||isempty(options.rho0));
options.isdf_threshold = 1e-3;
options.store_tensors=false;
options.aceconv=false;
options.exxmethod='ace';
options.x_gamma_extrapolation = false;
...
switch field
    ...
    case {'maxscfiter','maxdcmiter','maxinerscf','maxcgiter','maxeigsiter','brank','mixdim','maxphiiter'} % integer 
    ...
    case {'degree', 'dfrank', 'ncbands'}  %integer
        [validvalue, errmsg, errid] = nonNegInteger(field,value);
    case {'force','ishybrid'}  %boolean integer
        [validvalue, errmsg, errid] = nonNegInteger(field,value);
    case {'Vexx'}
        validvalue = 1;
    otherwise
...
if ~valid
    ...
    if (any(strcmp(lower(value),{'lobpcg';'eigs';'chebyfilt';'omm';'ppcg';'davidson';'davidson2';'davpcg'})))
       errid = '';
       errmsg = '';
    else
       valid = 0;
