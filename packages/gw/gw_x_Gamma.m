...
if isisdf
  ...
  output.msg('v0s', '%s', sprintf('[Exchange] ISDF Sigma_x: %s adaptive slot + get_rho_xalpha + tildeVq.', ex_label));
  if isempty(id_ex)
  ...
  if isempty(ex_data.tildeVq) || size(ex_data.tildeVq, 3) < nqibz
    output.msg('v1s', '%s', sprintf('[Exchange] Building tildeVq for %s id=%d ...', ex_label, id_ex));
  ...
  output.msg('v0s', '%s', '[Exchange] Dense G-space Sigma_x (SCATTER_Bamp).');
end
