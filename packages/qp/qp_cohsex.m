% License-Identifier: BSD-3-Clause
%
% Copyright (C) 2026
%
% Authors (see AUTHORS file for details): ZZ
%
% Last modified: 2026/05/20 ZZ

function E = qp_cohsex(config)
%QP_COHSEX  Quasiparticle COHSEX route: Eqp = Ex + Esx_x + Ecoh (package path).

  output.msg('v0s', '%s', '[COHSEX] Eqp = Ex + Esx_x + Ecoh (gw_x_k_packages + gw_cohsex_multi_k)...');
  t0 = tic;

  if config.FREQUENCY.frequency_dependence ~= -2
    Ex = gw_x_k_packages(config);
  else
    Ex = gw_x_Gamma(config);
  end
  
  switch config.FREQUENCY.frequency_dependence
    case -2
      [Esx_x, Ecoh] = gw_cohsex_test_Gamma(config);
    case -1
      [Esx_x, Ecoh] = gw_cohsex_multi_k(config);
    case 2
      if ~isfield(config, 'freqinfo')
        config = generate_frequency(config);
      end
      Esx_x = gw_fullfreq_cd_res_Gamma(config);
      Ecoh = gw_fullfreq_cd_int_Gamma(config);
      ry2ev = constant_map().ry2ev;
      Esx_x = Esx_x / ry2ev;
      Ecoh = Ecoh / ry2ev;
    otherwise
      error('qp_cohsex:InvalidFrequencyDependence', 'Invalid frequency dependence.');
  end
  nik = size(Ex, 2);
  Eqp = zeros(size(Ex));
  for ik = 1:nik
    Eqp(:, ik) = Ex(:, ik) + Esx_x(:, ik) + Ecoh(:, ik);
    if ~isempty(Eqp(:, ik))
      msg = sprintf( ...
        '[COHSEX] ik=%d/%d  band1: Eqp=%.6f  Ex=%.6f  Esx_x=%.6f  Ecoh=%.6f', ...
        ik, nik, Eqp(1, ik), Ex(1, ik), Esx_x(1, ik), Ecoh(1, ik));
      output.msg('v2s', '%s', msg);
    end
  end

  output.msg('v0s', '%s', sprintf('[COHSEX] Finished in %.2f s (nb=%d, nk=%d).', toc(t0), size(Eqp, 1), nik));

  E.Eqp = Eqp;
  E.Ex = Ex;
  E.Esx_x = Esx_x;
  E.Ecoh = Ecoh;
  E.Eqp0 = [];
  E.fout = '';

  try
    E.fout = qp_cohsex_fout(E, config);
    output.msg('v0s', '%s', sprintf('[COHSEX] Saved qp_cohsex table to: %s', E.fout));
  catch ME
    output.msg('v0s', '%s', sprintf('[COHSEX] Failed to write qp_cohsex output file: %s', ME.message));
  end
end
